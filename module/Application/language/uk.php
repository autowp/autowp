<?php return array (
  'specs/attrs/22/24/26/options/x' => 'X',
  'car-type/singleseater' => 'Single-seater',
  'car-type-rp/singleseater' => 'Single-seater',
  'page/36/ng-name' => 'Технічні характеристики',
  'catalogue/all-pictures-of' => 'Всі зображення',
  'catalogue/sport-of' => 'Спорт',
  'catalogue/related-with' => 'Пов\'язані з',
  'catalogue/other-pictures-of' => 'Інші зображення',
  'page/34/ng-name' => 'Всі зображення {item}',
  'page/138/ng-name' => '{brand} {vehicle_type} в хронологічному порядку',
  'page/14/ng-name' => '{brand} в хронологічному порядку',
  'page/208/ng-name' => 'Двигуни {brand}',
  'page/37/ng-name' => 'Концепти і прототипи {brand}',
  'page/40/ng-name' => '{brand} різне',
  'page/39/ng-name' => 'Логотипи {brand}',
  'page/15/ng-name' => 'Останні зображення {brand}',
  'perspective/chassis' => 'шасі',
  'brands/more-{count}-companies' => 'ще {count, plural, one {{count} компанія} few {{count} компанії} many {{count} компаній} other {{count} компаній}}',
  'perspective/mockup' => 'макет',
  'catalogue/specifications-of-{name}' => 'Технічні характеристики {name}',
  'moder/picture/edit-item' => 'Управління',
  'moder/picture/edit-picture-{id}' => 'Управління зображенням №{id}',
  'page/141/ng-name' => 'Додані зображення {brand}',
  'page/28/ng-name' => 'Всі зображення {group_name}',
  'page/102/ng-name' => 'Редактор характеристик {item_name}',
  'page/27/ng-name' => 'Технічні характеристики {group_name}',
  'page/136/name' => 'Про проект',
  'page/136/title' => 'About us',
  '{count} vehicles' => '{count, plural, one {{count} vehicle} other {{count} vehicles}}',
  'select' => 'select',
  'picture/text' => 'Всі зображення є чиєюсь власністю.

Якщо ви вважаєте, що користувачі сайту помилились, розмістивши дану картинку в цьому розділі, то було б чудово, якби ви написали про це в коментарях і вказали на помилку.

Поділитися своєю радістю, обговорити наболіле і просто потеревенити про автомобілі і не тільки ви можете на нашому [форумі](/forums/).',
  'specs/attrs/157/options/euro-6d' => 'Euro 6d',
  'specs/attrs/157/options/euro-5a' => 'Euro 5a',
  'specs/attrs/157/options/euro-5b' => 'Euro 5b',
  'item/type/9/add-sub-item' => 'Add sub copyright block',
  'item/type/9/name' => 'Copyright block',
  'item/type/9/new-item' => 'New copyright block',
  'item/type/9/name-plural' => 'Copyright blocks',
  'specs/attrs/22/23/options/common-rail' => 'Common Rail',
  'specs/attrs/22/23/options/common-rail/mechanical' => 'Common Rail (механічне впорскування)',
  'specs/attrs/22/23/options/common-rail/electronic' => 'Common Rail (електронне впорскування)',
  'specs/attrs/transmission/gearbox/reverse-gears' => 'кількість задніх передач',
  'specs/attrs/dynamic/max-reverse-speed' => 'максимальна швидкість (назад)',
  'specs/attrs/wheels/auto-pumping-tires' => 'Автопідкачка шин',
  'page/214/name' => 'Персони',
  'persons/content-persons' => 'Персони на фото',
  'mosts/most' => 'Самі',
  'persons/author-persons' => 'Автори',
  'mosts/among' => 'серед',
  'upload/select-files' => 'Вибрати файли',
  'upload/select-file' => 'Вибрати файл',
  'page/42/name' => 'Форуми',
  'page/213/name' => '%PERSON_NAME%',
  'page/51/name' => 'Нові фотографії',
  'forums/theme/all-other/description' => 'Інші теми, які не ввійшли до перелічених вище розділів',
  'inbox/all-brands' => 'Всі марки',
  'forums/theme/german-cars/description' => 'Audi, BMW, Mercedes-Benz, Opel, Volkswagen, ...',
  'forums/theme/italian-cars/description' => 'Alfa Romeo, Ferrari, Fiat, Lamborghini, Lancia, ...',
  'forums/theme/french-cars/description' => 'Peugeot, Renault, Citroën, Delahaye ...',
  'forums/theme/japan-cars/description' => 'Nissan, Toyota, Honda, Mazda, ...',
  'forums/theme/korean-cars/description' => 'Kia, Hyundai, ...',
  'forums/theme/chinese-cars/description' => 'Chery, Geely, Great Wall, ...',
  'forums/theme/russian-cars/description' => 'ВАЗ, ГАЗ, УАЗ, ...',
  'forums/theme/american-cars/description' => 'Cadillac, Chrysler, Chevrolet, Dodge, Ford, Hummer, Pontiac, ...',
  'forums/theme/british-cars/description' => 'Aston Martin, Bentley, MG, Rover, ...',
  'page/18/name' => '%PICTURE_NAME%',
  'page/23/name' => '%CATEGORY_NAME%',
  'page/26/name' => '%TWINS_GROUP_NAME%',
  'page/33/name' => '%CAR_NAME%',
  'page/43/name' => '%THEME_NAME%',
  'page/44/name' => '%TOPIC_NAME%',
  'page/58/name' => '%BRAND_NAME% %DPBRAND_NAME%',
  'page/59/name' => '%BRAND_NAME% %DESIGN_PROJECT_NAME%',
  'page/62/name' => '%USER_NAME%',
  'page/72/name' => '%PICTURE_NAME%',
  'page/78/name' => '%CAR_NAME%',
  'page/85/name' => '%BRAND_NAME%',
  'page/101/name' => '%ATTR_NAME%',
  'page/142/name' => '%ZONE_NAME%',
  'page/157/name' => '%VOTING_NAME%',
  'page/163/name' => '%NEW_ITEM_OF_TYPE%',
  'page/159/name' => '%MUSEUM_NAME%',
  'page/181/name' => '%FACTORY_NAME%',
  'page/165/name' => '%MOST_NAME% %BRAND_NAME%',
  'page/183/name' => '%PICTURE_NAME%',
  'page/184/name' => '%PICTURE_NAME%',
  'page/185/name' => '%CAR_NAME%',
  'page/187/name' => '%PICTURE_NAME%',
  'page/190/name' => '%PICTURE_NAME%',
  'page/191/name' => '%PICTURE_NAME%',
  'page/192/name' => '%PICTURE_NAME%',
  'page/193/name' => '%PICTURE_NAME%',
  'page/194/name' => '%PICTURE_NAME%',
  'page/10/title' => '%BRAND_NAME%',
  'page/18/title' => '%PICTURE_NAME%',
  'page/23/title' => '%CATEGORY_NAME%',
  'page/24/title' => '',
  'page/33/title' => '%CAR_NAME%',
  'page/26/title' => '%TWINS_GROUP_NAME%',
  'page/58/title' => '%BRAND_NAME% %DPBRAND_NAME%',
  'page/59/title' => '%BRAND_NAME% %DESIGN_PROJECT_NAME%',
  'page/61/title' => '',
  'page/85/title' => '%BRAND_NAME%',
  'page/89/title' => '',
  'page/90/title' => '',
  'page/91/title' => '',
  'page/97/title' => '',
  'page/104/title' => '',
  'page/124/title' => '',
  'page/165/title' => '%MOST_NAME% %BRAND_NAME%',
  'page/183/title' => '%PICTURE_NAME%',
  'page/184/title' => '%PICTURE_NAME%',
  'page/185/title' => '%CAR_NAME%',
  'page/187/title' => '%PICTURE_NAME%',
  'page/190/title' => '%PICTURE_NAME%',
  'page/191/title' => '%PICTURE_NAME%',
  'page/192/title' => '%PICTURE_NAME%',
  'page/193/title' => '%PICTURE_NAME%',
  'page/194/title' => '%PICTURE_NAME%',
  'page/2/breadcrumbs' => '',
  'page/10/breadcrumbs' => '%BRAND_NAME%',
  'page/18/breadcrumbs' => '%PICTURE_NAME%',
  'page/20/breadcrumbs' => '',
  'page/22/breadcrumbs' => '',
  'page/23/breadcrumbs' => '%CATEGORY_SHORT_NAME%',
  'page/24/breadcrumbs' => '',
  'page/25/breadcrumbs' => '',
  'page/26/breadcrumbs' => '%TWINS_GROUP_NAME%',
  'page/33/breadcrumbs' => '%SHORT_CAR_NAME%',
  'page/48/breadcrumbs' => '',
  'page/56/breadcrumbs' => '',
  'page/57/breadcrumbs' => '',
  'page/58/breadcrumbs' => '%DPBRAND_NAME%',
  'page/59/breadcrumbs' => '%DESIGN_PROJECT_NAME%',
  'page/61/breadcrumbs' => '',
  'page/85/breadcrumbs' => '%BRAND_NAME%',
  'page/87/breadcrumbs' => '',
  'page/89/breadcrumbs' => '',
  'page/90/breadcrumbs' => '',
  'page/91/breadcrumbs' => '',
  'page/97/breadcrumbs' => '',
  'page/104/breadcrumbs' => '',
  'page/105/breadcrumbs' => '',
  'page/107/breadcrumbs' => '',
  'page/114/breadcrumbs' => '',
  'page/124/breadcrumbs' => '',
  'page/130/breadcrumbs' => '',
  'page/138/breadcrumbs' => '%CAR_TYPE_NAME%',
  'page/153/breadcrumbs' => '%BRAND_NAME%',
  'page/154/breadcrumbs' => '%MOST_NAME%',
  'page/155/breadcrumbs' => '%CAR_TYPE_NAME%',
  'page/156/breadcrumbs' => '%YEAR_NAME%',
  'page/165/breadcrumbs' => '%MOST_NAME%',
  'page/166/breadcrumbs' => '%CAR_TYPE_NAME%',
  'page/167/breadcrumbs' => '%YEAR_NAME%',
  'page/183/breadcrumbs' => '%PICTURE_NAME%',
  'page/184/breadcrumbs' => '%PICTURE_NAME%',
  'page/185/breadcrumbs' => '%CAR_NAME%',
  'page/187/breadcrumbs' => '%PICTURE_NAME%',
  'page/190/breadcrumbs' => '%PICTURE_NAME%',
  'page/191/breadcrumbs' => '%PICTURE_NAME%',
  'page/192/breadcrumbs' => '%PICTURE_NAME%',
  'page/193/breadcrumbs' => '%PICTURE_NAME%',
  'page/194/breadcrumbs' => '%PICTURE_NAME%',
  'page/209/title' => '%DATE%',
  'page/209/name' => '%DATE%',
  'page/209/breadcrumbs' => '%DATE%',
  'page/210/name' => '%ITEM_NAME%',
  'test' => 'тест пройшов успішно',
  'and' => 'і',
  'ng/%1$s-pictures' => '{count, plural, one {{count} зображення} few {{count} зображень} many {{count} зображень} other {{count} зображень}}',
  'ng/%1$s-items' => '{count, plural, one {{count} одиниця даних} few {{count} одиниці даних} many {{count} одиниць даних} other {{count} одиниць даних}}',
  'ng/%1$s-comments' => '{count, plural, one {{count} коментар} few {{count} коментарів} many {{count} коментарів} other {{count} коментарів}}',
  '{count} messages' => '{count, plural, one {{count} повідомлення} other {{count} повідомлень}}',
  '{topics} topics and {messages} messages' => '{topics, plural, one {{topics} тема} few {{topics} теми} many {{topics} тем} other {{topics} тем}} и {messages, plural, one {{messages} повідомлення} few {{messages} повідомлень} many {{messages} повідомлень} other {{messages} повідомлень}}',
  'year' => 'Рік',
  'contributor' => 'упорядник',
  'month' => 'Місяць',
  'contributors' => 'упорядники',
  'deleted-user' => 'видалений користувач',
  'submit' => 'Надіслати',
  'layout/meta/description' => 'Енциклопедія автомобілів в зображеннях. WheelsAge.org',
  'layout/meta/keywords' => 'автомобіль, автомобіль, авто, авто',
  'layout/personal-messages' => 'Приватні повідомлення',
  'layout/brand' => 'WheelsAge.org',
  'ng-layout/you-have-new-messages' => '{messages, plural,
        one {У вас {messages} нове повідомлення}
        few {У вас {messages} нових повідомлень}
        many {У вас {messages} нових повідомлень}
        other {У вас {messages} нових повідомлень}
    }',
  'layout/search' => 'Пошук',
  'layout/footer' => 'Звертаємо вашу увагу на те, що всі матеріали, розміщені на сервері autowp.ru, додані відвідувачами сайту.

Адміністрація сайту не має інформації про правомірність публікування цих матеріалів.

Всі зображення розміщені в ознайомлювальних цілях.

[Повідомити про порушення авторських прав](mailto:autowp@yandex.ru)',
  'layout/language-contributing' => 'Бажаете допомогти?',
  'picture-preview/no-comments' => 'ні',
  'picture-preview/no-ratio' => 'ні',
  'picture-preview/crop-desc' => 'На зображенні виділена область автомобіля',
  'picture-preview/special-name' => 'Зображення зі спеціальним ім\'ям',
  'Resolution' => 'Роздільна здатність',
  'Filesize' => 'Розмір',
  'Ratio' => 'Оцінка',
  'Views' => 'Переглядів',
  'Comments count' => 'Коментарів',
  'rpm' => 'об/хв',
  'Nm' => 'Нм',
  'kW' => 'КВт',
  'hp' => 'к.с.',
  'next' => 'наступна',
  'previous' => 'попередня',
  'index/brands' => 'Компанії',
  'index/factories' => 'Заводи',
  'index/specifications' => 'Характеристики',
  'index/twins/description' => 'Ви знаєте, що Daewoo Matiz має аж цілих [шість клонів](/twins/group/118812)? А знаєте, що [спільного у Lotus і Kia](/twins/group/118750)? Впевнені, що вам здасться цікавим цей розділ.',
  'index/categories/description' => 'А чи знаєте ви, яка техніка допомагала дотримуватися правопорядку на вулицях світових міст в різний час? А на яких автомобілях їздив Папа Римський? Ні? Тоді вам точно в цей розділ.',
  'index/categories/mosts' => 'Бажаєте знати, який автомобіль був найшвидшим? Бажаєте знати, скільки важить наймогутніший кар\'єрний самоскид? Заходьте і дізнаєтесь!',
  'mosts/fastest/roadster' => 'Найшвидші родстери',
  'mosts/mighty/sedan/today' => 'Наймогутніші седани сьогодення',
  'mosts/dynamic/universal/2000-09' => 'Найдинамічніші універсали 2000-х років',
  'mosts/heavy/truck' => 'Найважчі вантажівки',
  'back' => 'назад',
  'forward' => 'вперед',
  'Picture of the day' => 'Зображення дня',
  'theme-of-day' => 'тема дня',
  'car-of-day' => 'автомобіль дня',
  'day ahead' => 'день вперед',
  'day ago' => 'день тому',
  'Cars by destination' => 'Автомобілі за призначенням',
  'All new pictures' => 'Всі нові зображення.',
  'brands/new-cars-of-brand' => 'Нові автомобілі {brand}',
  'all-link' => 'всі',
  'brands/pictures:' => 'зображень:',
  'item/type/1/name' => 'ТЗ',
  'item/type/1/name-plural' => 'ТЗ',
  'item/type/1/new-item' => 'Новий ТЗ',
  'item/type/1/add-sub-item' => 'Додати підрозділ автомобілів',
  'item/type/2/name' => 'Двигун',
  'item/type/2/name-plural' => 'Двигуни',
  'item/type/2/new-item' => 'Новий двигун',
  'item/type/2/add-sub-item' => 'Додати підрозділ двигунів',
  'item/type/3/name' => 'Категорія',
  'item/type/3/name-plural' => 'Категорії',
  'item/type/3/new-item' => 'Нова категорія',
  'item/type/4/name' => 'Близнюки',
  'item/type/3/add-sub-item' => 'Додати підкатегорію',
  'item/type/4/name-plural' => 'Близнюки',
  'item/type/4/new-item' => 'Нова група близнят',
  'item/type/4/add-sub-item' => 'Додати підкатегорію близнят',
  'item/type/5/name' => 'Марка',
  'item/type/5/name-plural' => 'Марки',
  'item/type/5/new-item' => 'Нова марка',
  'item/type/5/add-sub-item' => 'Додати суббренд',
  'item/type/6/name' => 'Завод',
  'item/type/6/name-plural' => 'Заводи',
  'item/type/6/new-item' => 'Новий завод',
  'item/type/6/add-sub-item' => 'Додати підкатегорію заводів',
  'item/type/7/name' => 'Музей',
  'item/type/7/name-plural' => 'Музеї',
  'item/type/7/new-item' => 'Новий музей',
  'item/type/7/add-sub-item' => 'Додати підкатегорію музеїв',
  'item/type/8/name' => 'Персона',
  'item/type/8/name-plural' => 'Персони',
  'item/type/8/new-item' => 'Нова персона',
  'item/type/8/add-sub-item' => 'Додати суб-персону',
  'unsorted' => 'Несортоване',
  'engines' => 'Двигуни',
  'concepts and prototypes' => 'Концепти та прототипи',
  'logotypes' => 'Логотипи',
  'mixed' => 'Різне',
  'count 0' => 'ні',
  'carlist/no-photo' => 'Зображення відсутні',
  'carlist/no-photo/add' => 'Додати зображення',
  'carlist/all pictures' => 'всі зображення',
  'carlist/details' => 'детальніше',
  'carlist/twins' => 'близнюки',
  'carlist/produced/one' => 'Виготовлено у єдиному екземплярі',
  'ng/carlist/produced/%1$s' => '{count, plural, one {Виготовлена {count} одиниця} few {Виготовлено {count} одиниць} many {Виготовлені {count} одиниці} other {Виготовлені {count} одиниці}}',
  'ng/carlist/produced-about/%1$s' => '{count, plural, one {Виготовлена приблизно {count} одиниця} few {Виготовлено приблизно {count} одиниць} many {Виготовлені приблизно {count} одиниць} other {Виготовлені приблизно {count} одиниць}}',
  'carlist/specifications' => 'технічні характеристики',
  'ng/carlist/designed by' => 'проект',
  'carlist/designed by %s' => 'дизайн %s',
  'carlist/edit-specs' => 'редагувати технічні характеристики',
  'carlist/years' => 'роки виробництва',
  'carlist/model-years' => 'модельний рік',
  'picturelist/engine' => 'Двигун',
  'picturelist/engine-%s' => '%s двигун',
  'picturelist/factory' => 'Завод',
  'present-time-abbr' => 'н.ч.',
  'most/fastest' => 'найшвидший',
  'most/fastest/param' => 'максимальна швидкість',
  'most/slowest/param' => 'максимальна швидкість',
  'most/slowest' => 'найповільніший',
  'most/dynamic' => 'динамічні',
  'most/static' => 'інертні',
  'most/dynamic/param' => 'розгін',
  'most/static/param' => 'розгін',
  'most/mighty' => 'потужні',
  'most/mighty/param' => 'потужність двигуна',
  'most/weak' => 'слабкі',
  'most/weak/param' => 'потужність двигуна',
  'most/big-engine' => 'великі двигуни',
  'most/big-engine/param' => 'об\'єм двигуна',
  'most/small-engine' => 'меленькі двигуни',
  'most/small-engine/param' => 'об\'єм двигуна',
  'most/nimblest' => 'спритні',
  'most/nimblest/param' => 'діаметр розвороту',
  'most/economical' => 'економічні',
  'most/economical/param' => 'розхід палива у змішаному циклі',
  'most/gluttonous' => 'ненажерливі',
  'most/gluttonous/param' => 'розхід палива у змішаному циклі',
  'most/clenaly' => 'екологічні',
  'most/clenaly/param' => 'викиди',
  'most/dirty' => 'не екологічні',
  'most/dirty/param' => 'викиди',
  'most/heavy' => 'важкі',
  'most/heavy/param' => 'споряджена маса',
  'most/lightest' => 'легкі',
  'most/lightest/param' => 'споряджена маса',
  'most/longest' => 'довгі',
  'most/longest/param' => 'довжина',
  'most/shortest' => 'короткі',
  'most/shortest/param' => 'довжина',
  'most/widest' => 'широкі',
  'most/widest/param' => 'ширина',
  'most/narrow' => 'вузькі',
  'most/narrow/param' => 'ширина',
  'most/highest' => 'високі',
  'most/highest/param' => 'висота',
  'most/lowest' => 'низькі',
  'most/lowest/param' => 'висота',
  'most/air' => 'аеродинамічні',
  'most/air/param' => 'коефіцієнти аеродинамічного опору',
  'most/antiair' => 'антиаеродинамічні',
  'most/antiair/param' => 'коефіцієнти аеродинамічного опору',
  'most/bigwheel' => 'великі колеса',
  'most/bigwheel/param' => 'розміри коліс',
  'most/smallwheel' => 'маленькі колеса',
  'most/smallwheel/param' => 'розміри коліс',
  'most/bigbrakes' => 'великі гальма',
  'most/bigbrakes/param' => 'розмір гальм',
  'most/smallbrakes' => 'маленькі гальма',
  'most/smallbrakes/param' => 'розмір гальм',
  'most/bigclearance' => 'великий кліренс',
  'most/bigclearance/param' => 'дорожній просвіт',
  'most/smallclearance' => 'малий кліренс',
  'most/smallclearance/param' => 'дорожній просвіт',
  'mosts/sidebar/mostly…' => 'Самі ...',
  'mosts/sidebar/between…' => 'Серед ...',
  'mosts/period/before1920' => ' до 1920',
  'mosts/period/1920-29' => '1920-29их',
  'mosts/period/1930-39' => '1930-39их',
  'mosts/period/1940-49' => '1940-49их',
  'mosts/period/1950-59' => '1950-59их',
  'mosts/period/1960-69' => '1960-69их',
  'mosts/period/1970-79' => '1970-79их',
  'mosts/period/1980-89' => '1980-89их',
  'mosts/period/1990-99' => '1990-99их',
  'mosts/period/2000-09' => '2000-09их',
  'mosts/period/2010-17' => '2010-17их',
  'mosts/period/present' => 'нашого часу',
  'mosts/period/all-time' => 'за всю історію',
  'upload/image-file' => 'Файл картинки',
  'upload/note' => 'Примітка до зображення',
  'upload/picture/crop' => 'кадрувати',
  'upload/picture/cropped-to' => 'кадровано {resolution}',
  'upload/select/unsorted' => 'не відсортовано',
  'upload/select/unsorted-long' => 'не відсортовано (автомобіль чи двигун, який відсутній на сайті)',
  'upload/select/mixed' => 'різне',
  'upload/select/mixed-long' => 'різне (кілька автомобілів на одній картинці)',
  'upload/select/logo' => 'логотип',
  'upload/select/engines' => 'Двигуни',
  'upload/select/concepts' => 'Концепти та прототипи',
  'upload/select/other-modification' => 'інша модифікація',
  'upload/add-picture' => 'Додати в обране',
  'upload/select-another' => 'вибрати іншу рубрику',
  'upload/description' => 'Якщо у вас є зображення, яких ви не знайшли на цьому сайті і вони відповідають тематиці сайту, то ви можете додати їх в наший каталог, скориставшись формою, розміщеною нижче.

Було б чудово, якби ви вказали, що за автомобіль зображений на картинці, щоб полегшити її додавання і уникнути помилки.

Вимоги, які пред\'являються до додаваних картинок:

* Формат [JPEG](http://ru.wikipedia.org/wiki/JPEG) або [PNG](http://ru.wikipedia.org/wiki/PNG). Розмір від: 640×360 до 10000×10000.
* Зображення повинно по можливості бути максимально доступної якості, не містити різноманітних артефактів зміни розміру або компресії (розмиття, нечіткості, розпливчатості кольорів, ребристості прямих ліній і т.д.). Виключення становлять рідкісні фото(виробника), скани із проспектів. Рішення про залишення такого фото на сайті приймає модераторський склад.
* Відсутність надписів і логотипів сторонніх сайтів, нанесених не авторами/власниками матеріалу.
* Автомобіль повинен мати початковий вигляд як при виході з заводу. Не повинні бути присутніми  ознаки <em>саморобного тюнінгу</em>, бажано відсутність змін, пов\'язаних зі старінням автомобіля.
* Основною і єдиною темою зображення повинен бути автомобіль, двигун, логотип автокомпанії, автомобільний завод або музей. Наявність в кадрі сторонніх предметів або людей - небажано.
* Любительські фотографії вашого автомобіля, автомобіля ваших друзів або будь-кого, хто зустрівся вам на вулиці, а також фотографії з виставок - не будуть прийняті на сайт 100%. Якщо тільки ви не професіонал.
* Для перших фотографій автомобіля можливі понижені вимоги.

**Переконливе прохання**, перед додаванням зображень, впевніться в їх відсутності в каталозі.

Звертаємо вашу увагу на те, що додавання картинок модерується. Тобто всі картинки перед додаванням в каталог будуть перевірені ведучими сайту.',
  'upload/only-registered' => 'Тільки зареєстрований користувач може завантажувати зображення

[Авторизуйтесь](/login) або [зареєструйтесь](/signup)',
  'catalogue/other-pictures-of-%1$s' => 'Інші зображення %1$s',
  'catalogue/all-pictures-of-%1$s' => 'Всі зображення %1$s',
  'catalogue/brand/logo-of-%s' => 'логотип %s',
  'catalogue/brand/links' => 'Посилання',
  'catalogue/brand/popular-images' => 'Популярні картинки',
  'catalogue/brand/new-pictures' => 'Нові фото',
  'catalogue/brand/inbox/by-brand' => 'Інбокс бренду',
  'catalogue/brand/factories' => 'Заводи',
  'catalogue/link-category/official' => 'Офіційні сайти',
  'catalogue/link-category/helper' => 'Допомогли в створенні проекта',
  'catalogue/link-category/club' => 'Автомобільні клуби',
  'catalogue/link-category/default' => 'Інші',
  'catalogue/chronological' => 'Хронологія',
  'catalogue/related' => 'Пов\'язані',
  'catalogue/related-with-%1$s' => 'Пов\'язані з %1$s',
  'catalogue/sport' => 'Спорт',
  'catalogue/design' => 'Дизайн',
  'catalogue/stock-model' => 'стокова модель',
  'catalogue/sub-model' => 'підвид',
  'catalogue/sport-of-%1$s' => '%1$s Спорт',
  'catalogue/specifications-of-%1$s' => 'Технічні характеристики %1$s',
  'catalogue/specifications' => 'Технічні характеристики',
  'catalogue/other-photos' => 'Інші фото',
  'catalogue/section/moto' => 'Мото',
  'catalogue/section/tractors' => 'Трактори',
  'catalogue/section/buses' => 'Автобуси',
  'catalogue/section/trucks' => 'Вантажівки',
  'catalogue/section/engines' => 'Двигуни',
  'catalogue/engine-menu/vehicles' => 'Автомобілі',
  'catalogue/engine-menu/pictures' => 'Зображення',
  'catalogue/specifications/contributors:' => 'Укладачі:',
  'catalogue/specifications/disclaimer' => 'Інформація зібрана учасниками проекту. Якщо ви знайшли помилку або бажаєте заповнити пробіли, ви можете зробити це, пройшовши процедуру реєстрації на сайті.',
  'catalogue/vehicle/inbox' => 'інбокс',
  'catalogue/vehicle/comments' => 'коментарі',
  'catalogue/engine/add-engine' => 'додати двигун',
  'catalogue/engine/mounted-on:' => 'Встановлювався на:',
  'factories/factory-items' => 'Продукція',
  'factories/factory/pictures' => 'Зображення',
  'comments/title' => 'Коментарі',
  'comments/reply' => 'відповісти',
  'comments/what-to-do-for-write-comments' => 'Щоб мати можливість залишати коментарі, необхідно [зареєструватися або ввійти на сайт з допомогою облікового запису в одній з соціальних мереж](/login)',
  'comments/it-requires-attention-of-moderators' => 'Потрібна увага модераторів',
  'comments/form-title' => 'Додати коментар',
  'comments/message' => 'Повідомлення',
  'comments/send' => 'Відправити',
  'comments/cancel' => 'Відмінити',
  'comments/need-wait-text' => 'На жаль вам потрібно трішки почекати, щоб отримати можливість залишати коментарі.

Це пов\'язано з наявністю порогу "повідомлень в годину", необхідного для боротьби зі спамом і деякими іншими негативними моментами.

Наступний коментар ви зможете залишити в "%s", а поки у вас є час уточнити ту думку, яку ви хотіли донести до громадськості:',
  'comments/author/anonymous' => 'Анонім',
  'comments/message-deleted-by-admin' => 'Повідомлення видалено адміністрацією сайту',
  'comments/message/delete' => 'Видалити',
  'comments/message/restore' => 'Відновити',
  'comments/message/move' => 'Перемістити',
  'comments/vote/no-more-votes' => 'На сьогодні у вас закінчилися голоси',
  'comments/subscribe' => 'Підписатися на повідомлення',
  'comments/unsubscribe' => 'Відписатися від повідомлень',
  'perspective/front' => 'перед',
  'perspective/back' => 'зад',
  'perspective/left' => 'лівий бік',
  'perspective/right' => 'правий бік',
  'perspective/interior' => 'інтер\'єр',
  'perspective/front-panel' => 'передня панель',
  'perspective/3/4-left' => '¾ лівий бік',
  'perspective/3/4-right' => '¾ правий бік',
  'perspective/cutaway' => 'cutaway',
  'perspective/front-strict' => 'перед (строго)',
  'perspective/left-strict' => 'лівий бік (строго)',
  'perspective/right-strict' => 'правий бік (строго)',
  'perspective/back-strict' => 'зад (строго)',
  'perspective/n/a' => 'n/a',
  'perspective/label' => 'шильдик',
  'perspective/upper' => 'зверху',
  'perspective/under-the-hood' => 'під капотом',
  'perspective/upper-strict' => 'зверху (строго)',
  'perspective/bottom' => 'знизу',
  'perspective/dashboard' => 'комбінація приладів',
  'perspective/boot' => 'багажник',
  'perspective/logo' => 'логотип',
  'perspective/mascot' => 'маскот',
  'perspective/sketch' => 'скетч',
  'perspective/mixed' => 'різне',
  'perspective/exterior-details' => 'деталі екстер\'єру',
  'login/sign-in' => 'Вхід',
  'login/sign-out' => 'Вихід',
  'login/login-or-email' => 'логін або e-mail',
  'login/captcha' => 'введіть код захисту',
  'login/registration' => 'Реєстрація',
  'login/forgot-password?' => 'Забули свій пароль?',
  'login/success-text' => 'Раді знову вас вітати на нашому сайті',
  'login/remember' => 'запам\'ятати',
  'login/if-you-lose-password' => 'Якщо Ви забули свій логін чи пароль, Ви можете скористатися [формою відновлення по e-mail](/restore-password)',
  'login/if-you-not-registered' => 'Якщо Ви ще не зареєстровані на сайті, то зможете [зробити це зараз](/signup)',
  'login/sign-in-using-account' => 'Ввійти, використовуючи акаунт',
  'login/login-or-password-is-incorrect' => 'Логін або пароль неправильний',
  'login/user-%value%-not-found' => 'Користувач з іменем або e-mail "%value%" не знайдений',
  'registration/if-you-registered' => 'Якщо ви вже зареєстровані, то вам не потрібно робити це знову, а потрібно лише ввести свій логін і пароль в формі справа.',
  'account/personal-message/reply' => 'відповісти',
  'account/personal-message/delete' => 'видалити',
  'account/personal-message/entire-dialog' => 'вся переписка',
  'account/personal-message/system-notification' => 'Системне повідомлення',
  'account/personal-message/delete-all-sent' => 'Видалити всі відправлені повідомлення',
  'account/personal-message/delete-all-system' => 'Видалити всі повідомлення',
  'account/personal-message/new' => 'нове',
  'account/personal-message/sent' => 'Повідомлення відправлено',
  'account/profile/personal' => 'Особисті дані',
  'account/profile/photo' => 'Фото',
  'account/profile/select-photo' => 'Вибрати фотографію',
  'account/profile/photo/saved' => 'Фото збережене',
  'account/profile/photo/deleted' => 'Фото видалене',
  'account/profile/use-gravatar' => 'Ви можете завантажити своє фото, або використати сервіс [Gravatar](http://gravatar.com/)',
  'account/profile/photo/delete' => 'Видалити фото',
  'account/profile/other' => 'Інше',
  'account/profile/votes-per-day' => 'Голосів у день',
  'account/profile/votes-left' => 'Голосів залишилось',
  'account/profile/timezone' => 'Часовий пояс',
  'account/profile/language' => 'Мова',
  'account/profile/saved' => 'Дані збережені',
  'account/access/change-password' => 'Змінити пароль',
  'account/access/change-password/current' => 'Поточний',
  'account/access/change-password/new' => 'Новий',
  'account/access/change-password/new-confirm' => 'Новий (підтвердження)',
  'account/access/change-password/current-password-is-incorrect' => 'Поточний пароль введений неправильно',
  'account/access/change-password/saved' => 'Пароль успішно змінений',
  'account/access/self-delete' => 'Видалити обліковий запис',
  'account/access/self-delete/password-is-incorrect' => 'Пароль введений неправильно',
  'account/specs/weight' => 'Вага',
  'account/specs/conflicts/filter/all' => 'Все',
  'account/specs/conflicts/filter/i-disagree' => 'Я не згідний',
  'account/specs/conflicts/filter/do-not-agree-with-me' => 'Не згідні зі мною',
  'account/specs/conflicts/filter/errors' => 'Помилки',
  'account/specs/conflicts/title/object' => 'Об\'єкт',
  'account/specs/conflicts/title/object/engine' => 'Двигун',
  'account/specs/conflicts/title/parameter' => 'Характеристика',
  'account/specs/conflicts/title/my-value' => 'Моє значення',
  'account/specs/conflicts/title/other-values' => 'Інші значення',
  'account/specs/conflicts/my-value/none' => 'нема',
  'account/email/your-current-email:' => 'Ваша поточна адреса e-mail: ',
  'account/email/your-dont-enter-email' => 'Ви не вказали адресу e-mail',
  'account/email/text' => 'На цій сторінці Ви можете змінити свой e-mail

На нову адресу буде надісланий лист з інструкціями для її підтверждення',
  'account/accounts/add' => 'Додати обліковий запис',
  'account/accounts/remove' => 'Видалити',
  'account/accounts/removed' => 'Обліковий запис видалений',
  'account/accounts/connect-failed' => 'Не вдалося прив\'язати обліковий запис. Можливо виникла помилка або доступ до додатку заборонений користувачем.',
  'account/accounts/disconnect-failed' => 'Не вдалося видалити обліковий запис.

Таке буває, якщо прив\'язаний обліковий запис є єдиним способом авторизації, тобто не заданий e-mail або інші облікові записи.',
  'account/delete/text' => 'Ми шкодуємо, що вам довелося опинитися на цій сторінці, і ви плануєте покинути наший сайт назавжди, але не можемо вам в цьому відмовити.

Натиснувши на червону кнопку, всі дані, які стосуються вас, будуть знеособлені або видалені.

Цей процес незворотній, тому добре подумайте і не приймайте поспішних рішень.',
  'account/deleted/text' => 'Обліковий запис видалений.
Бажаємо вам удачі.',
  'feedback/title' => 'Зворотній зв\'язок',
  'feedback/name' => 'Ваше ім\'я',
  'feedback/message' => 'Повідомлення',
  'feedback/donate-text' => 'Якщо ви прийшли сюди, щоб виразити слова підтримки, є можливість [виразити її в твердій валюті](/donate)',
  'feedback/sent' => 'Ваше повідомлення відправлено',
  'map/museums-and-factories' => 'Музеї і заводи',
  'picture/image-specifications' => 'Характеристики зображення',
  'picture/added-by:' => 'Додав:',
  'picture/location' => 'Координати',
  'picture/status:' => 'Статус зображення:',
  'picture/status/inbox' => 'Рішення не винесене',
  'picture/status/accepted' => 'Прийнято',
  'picture/status/removing' => 'Поставлено в чергу на видалення',
  'picture/propose-image-replacement' => 'Запропонувати заміну зображенню',
  'picture/moderators-about-this-picture' => 'Модератори про це фото',
  'picture/other-languages' => 'На інших мовах',
  'picture/picture-suggested-to-replace' => 'Фото запропоноване на заміну',
  'picture/that-engine-is-mounted-to-vehicle' => 'Цей двигун встановлювався на автомобілі',
  'picture/that-engine-is-mounted-to-vehicles:' => 'Цей двигун встановлювався на автомобілі:',
  'picture/factory-details' => 'Детальніше про завод ...',
  'picture/on-that-factory-produced-vehicle' => 'На цьому заводі вироблявся такий автомобіль, як',
  'picture/on-that-factory-produced-vehicles:' => 'На цьому заводі вироблявся такий автомобіль, як:',
  'picture/on-that-factory-produced-vehicles/and' => 'і',
  'picture/on-that-factory-produced-vehicles/and-other' => 'інші ...',
  'user/name' => 'Ім\'я',
  'user/password' => 'Пароль',
  'user/password-confirm' => 'Пароль (знову)',
  'users/user/known-as:' => 'Представився як:',
  'users/user/before-known-as:' => 'раніше відомий як:',
  'users/user/type:' => 'Тип облікового запису:',
  'users/user/type/moderator' => 'Модератор',
  'users/user/type/visitor' => 'Відвідувач',
  'users/user/upload-pictures:' => 'Додав зображення:',
  'users/user/pictures-left-on-site:' => 'з них на сайті залишилось:',
  'users/comments/order/new' => 'Нові',
  'users/comments/order/old' => 'Старі',
  'users/comments/order/positive' => 'Позитивні',
  'users/comments/order/negative' => 'Негативні',
  'users/for-moderators' => 'Для модераторів',
  'users/for-moderators/remove-photo' => 'Видалити фото користувача',
  'users/for-moderators/remove-user' => 'Видалити користувача',
  'users/for-moderators/last-visit-from-ip' => 'Останній раз заходив з IP-адреси:',
  'ban/that-address-is-banned' => 'Цю адресу забанив',
  'ban/until' => 'до',
  'ban/unban' => 'розбанити',
  'ban/ban' => 'Забанити',
  'ban/reason' => 'Причина',
  'ban/period' => 'На період',
  'ban/period/hour' => 'година',
  'ban/period/2-hours' => '2 години',
  'ban/period/4-hours' => '4 години',
  'ban/period/8-hours' => '8 годин',
  'ban/period/16-hours' => '16 годин',
  'ban/period/day' => 'день',
  'ban/period/2-days' => '2 дня',
  'users/user/recent-activity' => 'Активність на сайті',
  'users/user/recent-uploads' => 'Останні зображення',
  'users/user/recent-comments' => 'Останні коментарі',
  'users/user/registration-date' => 'Зареєстрований',
  'users/user/last-visited' => 'Останній раз був на сайті',
  'users/user/send-personal-message' => 'Відправити особисте повідомлення',
  'users/user/log' => 'Журнал подій користувача',
  'users/user/uploaded-pictures' => 'Додані зображення',
  'users/user/not-upload-pictures' => 'Не додавав зображень',
  'users/rating/specifications' => 'Технічні характеристики',
  'users/rating/pictures' => 'Зображення',
  'users/rating/likes' => 'Лайки',
  'users/rating/picture-likes' => 'Лайки до фото',
  'users/rating/specs-volume' => 'Характеристики',
  'users/rating/weight' => 'Вага',
  'users/registration/email-confirm-subject' => 'Реєстрація на %1$s',
  'users/registration/email-confirm-message' => 'Привіт.
Ви зареєструвались на сайті %1$s
Ваші реєстраційні дані:
E-mail: %2$s
Для підтвердження реєстрації і вказаної вами адреси електронної пошти, вам необхідно перейти за посиланням %3$s

Якщо ви не реєструвалися на сайті, то просто видаліть це повідомлення

З Повагою, Робот %4$s',
  'users/registration/email-confirm/token-not-found' => 'Посилання, по якому ви перейшли на сайт -
 застаріло',
  'users/registration/email-confirm/success-text' => 'Ваша адреса електронної пошти успішно підтведжена.',
  'users/registration/success/email-sent' => 'На електронну адресу, вказану при реєстрації, відправлено лист з інструкціями для підтвердження адреси e-mail.',
  'users/change-email/confirm-subject' => 'Підтвердження e-mail на %1$s',
  'users/change-email/confirm-message' => 'Привіт.

Ви або хтось інший запросили зміну контактної адреси свого акаунта на сайті %1$s на %2$s
Для підтвердження вказаної дії, вам необхідно перейти за посиланням %3$s

Якщо повідомлення потрапило до вас через помилку - просто видаліть його

З Повагою, Робот %1$s
',
  'users/change-email/confirmation-message-sent' => 'E-mail with a confirmation code was sent',
  'user/add-to-contacts' => 'Додати до контактів',
  'user/remove-from-contacts' => 'Видалити з контактів',
  'donate/text' => 'Не дивлячись на те, що ми позиціонуємо нашу енциклопедію як некомерційну, зовсім обходитися без грошей неможливо. Зміст сайту потребує деяких розходів.

Якщо у вас є бажання і можливість підтримати нас, ви можете зробити це, відправивши нам трішки грошей через PayPal або Yandex.Money.

Разом з пожертвами ви можете передати нам пару рядків про те, куди вам хотілось би бачити рух сайту і ми будемо відчувати себе трішки більше зобов\'язаними.

Ми не збираємо інформацію про пожертвувачах, але, якщо ви наполягаєте, ми можемо відмітити ваш внесок.

В знак вдячності ми даємо можливість [вибрати наступний автомобіль дня](/donate/vod)',
  'donate/target' => 'На роботу сайту',
  'donate/project' => 'WheelsAge.org',
  'donate/comment-hint' => 'Ваші побажання',
  'donate/success-text' => 'Ми вдячні вам за надану підтримку.

Постараємося вас не розчарувати.',
  'donate/vod/text' => 'Те невелике, що ми можемо запропонувати взамін фінансової допомоги для існування сайту – це можливість вибирати найближчий "автомобіль дня".

Для цього слідуйте інструкціям нижче.

Ми будемо раді, якщо ви захочете [здійснити безкорисну пожертву](/donate) на вільну суму.',
  'donate/vod/select-date' => '1. Виберіть дату',
  'donate/vod/date-busy' => 'зайнято',
  'donate/vod/select-item' => '2. Виберіть автомобіль',
  'donate/vod/select-item-text' => 'На роль автомобіля дня підійде майже будь-який, із наявних, але є деякі обмеження:

- автомобіль повинен бути вибраний вперше
- автомобіль повинен мати як мінімум 3 фотографії',
  'donate/vod/select-item/selection:' => 'Вибір:',
  'donate/vod/select-item/select' => 'Вибрати ...',
  'donate/vod/preview' => '3. Попередній перегляд',
  'donate/vod/anonymous' => 'Анонімно',
  'donate/vod/with-name' => 'З вказанням імені (потрібна реєстрація)',
  'donate/vod/make-donation' => '4. Зробіть пожертву',
  'donate/vod/order-message' => 'WheelsAge.org: автомобіль дня',
  'donate/vod/order-target' => 'Order %s',
  'donate/vod/message' => 'Ваші побажання',
  'donate/vod/price:' => 'Сума:',
  'donate/vod/price/currency' => 'руб.',
  'donate/vod/price-aroud' => 'приблизно {price}',
  'donate/vod/method/credit-card' => 'Банківська карта',
  'donate/vod/method/mobile' => 'Рахунок мобільного оператора',
  'donate/vod/method/yandex.money' => 'Яндекс.Гроші',
  'donate/vod/send' => 'Оплатити',
  'donate/vod/success-text' => 'Найближчим часом інформація про пожертву автоматично надійде до нас і ваш вибір буде здійснено.

Вдячні вам за надану підтримку.

Постараємося вас не розчарувати.',
  'donate/vod/wants-to-select?' => 'Бажаєте вибрати наступний?',
  'message/user-cancel-car-engine' => '%1$s відмінив двигун %2$s для автомобіля %3$s ( %4$s )',
  'car-type/roadster' => 'Родстер',
  'car-type/spyder' => 'Спайдер',
  'car-type/cabriolet' => 'Кабріолет',
  'car-type/cabrio-coupe' => 'Кабріо-купе',
  'car-type/targa' => 'Тарга',
  'car-type/coupe' => 'Купе',
  'car-type/sedan' => 'Седан',
  'car-type/hatchback' => 'Хетчбек',
  'car-type/crossover' => 'Кросовер',
  'car-type/universal' => 'Універсал',
  'car-type/limousine' => 'Лімузин',
  'car-type/pickup' => 'Пікап',
  'car-type/caravan' => 'Легковий фургон',
  'car-type/offroad' => 'Позашляховик',
  'car-type/minivan' => 'Мінівен',
  'car-type/van' => 'Фургон',
  'car-type/truck' => 'Вантажівка',
  'car-type/bus' => 'Автобус',
  'car-type/phaeton' => 'Фаетон',
  'car-type/4door-hardtop' => '4х дв. Хардтоп',
  'car-type/landau' => 'Ландо',
  'car-type/offroad-cabrio' => 'Позашляховик-кабріолет',
  'car-type/liftback-coupe' => 'Ліфтбек-купе',
  'car-type/liftback-sedan' => 'Ліфтбек-седан',
  'car-type/2door-hardtop' => '2х дв. Хардтоп',
  'car-type/minibus' => 'Мікроавтобус',
  'car-type/car' => 'Легковий автомобіль',
  'car-type/multiplex-bus' => 'Багатосекційний автобус',
  'car-type/offroad-short' => 'Короткобазний позашляховик',
  'car-type/brougham' => 'Брогхем',
  'car-type/fastback-sedan' => 'Фастбек-седан',
  'car-type/fastback-coupe' => 'Фастбек-купе',
  'car-type/tonneau' => 'Тонно',
  'car-type/2-floor-bus' => '2-х поверховий автобус',
  'car-type/town-car' => 'Таун кар',
  'car-type/barchetta' => 'Баркетта',
  'car-type/moto' => 'Мото',
  'car-type/tractor' => 'Трактор',
  'car-type/tracked' => 'Гусенична платформа',
  'car-type-rp/roadster' => 'Родстерів',
  'car-type-rp/spyder' => 'Спайдерів',
  'car-type-rp/cabriolet' => 'Кабріолетів',
  'car-type-rp/cabrio-coupe' => 'Кабріо-купе',
  'car-type-rp/targa' => 'Тарг',
  'car-type-rp/coupe' => 'Купе',
  'car-type-rp/sedan' => 'Седанів',
  'car-type-rp/hatchback' => 'Хетчбеків',
  'car-type-rp/crossover' => 'Кросоверів',
  'car-type-rp/universal' => 'Універсалів',
  'car-type-rp/limousine' => 'Лімузинів',
  'car-type-rp/pickup' => 'Пікапів',
  'car-type-rp/caravan' => 'Легкових фургонів',
  'car-type-rp/offroad' => 'Позашляховиків',
  'car-type-rp/minivan' => 'Мінівенів',
  'car-type-rp/van' => 'Фургонів',
  'car-type-rp/truck' => 'Вантажівок',
  'car-type-rp/bus' => 'Автобусів',
  'car-type-rp/phaeton' => 'Фаетонів',
  'car-type-rp/4door-hardtop' => '4-х дв. Хардтопів',
  'car-type-rp/landau' => 'Ландо',
  'car-type-rp/offroad-cabrio' => 'Позашляховиків-кабріолетів',
  'car-type-rp/liftback-coupe' => 'Ліфтбек-купе',
  'car-type-rp/liftback-sedan' => 'Ліфтбек-седанів',
  'car-type-rp/2door-hardtop' => '2-х дв. Хардтопів',
  'car-type-rp/minibus' => 'Мікроавтобусів',
  'car-type-rp/car' => 'Легкових автомобілів',
  'car-type-rp/multiplex-bus' => 'Багатосекційних автобусів',
  'car-type-rp/offroad-short' => 'Короткобазних позашляховиків',
  'car-type-rp/brougham' => 'Брогхемів',
  'car-type-rp/fastback-sedan' => 'Фастбек-седанів',
  'car-type-rp/fastback-coupe' => 'Фастбек-купе',
  'car-type-rp/tonneau' => 'Тонно',
  'car-type-rp/2-floor-bus' => '2-х поверхових автобусів',
  'car-type-rp/town-car' => 'Таун карів',
  'car-type-rp/barchetta' => 'Баркет',
  'car-type-rp/moto' => 'Мото',
  'car-type-rp/tractor' => 'Тракторів',
  'car-type-rp/tracked' => 'Гусеничних платформ',
  'about/text' => '### Наші люди

Наш проект своїм існуванням завдячує людям, які приходять сюди і вкладають свій час і знання.

Хтось додає матеріали, а хтось допомагає знайти помилки в уже наявних. Хтось спеціалізується на конкретній марці, а хтось встигає за всіма. Хтось без особливої уваги зі сторони наповнює сайт крок за кроком, а хтось збирає овації рідкісними, але пекучими фото.

Нас багато і ми різні, і це прекрасно. Ось лиш деякі з нас:

%users%

#### "Кольорова диференціація штанів"

Так склалось, що ми виділяємо деяких наших людей особливим кольором - зеленим. Не просто так - це особлива мітка. Знайте, якщо ви бачите когось із "зелених", ви завжди можете схопити його і спитати що завгодно про наш проект, тому що "зелені" - це найчуйніші і найзацікавленіші в житті проекту люди.

Деяка частина "зелених" наділена модераторськими функціями.

### Діалог і зворотній зв\'язок

Якщо у вас є якісь зауваження, пропозиції або інші думки, ви можете озвучити їх на [форумі](/forums/), задати особисто через систему обміну повідомленнями або написати в "[зворотній зв\'язок](/feedback)" адміністрації сайту.

Якщо у вас є питання про розміщення реклами, обміну посиланнями або просуванні вашого продукту іншими способами, всі вони мають єдину відповідь: ми не розміщуємо рекламу.

### Числа

Так склалося, що ми любимо тішити своє марнославство великими числами, а також всім їх показувати. Вашій увазі деякі з них:

* на сайті більше %total-pictures% зображень, %total-vehicles% автомобілів, що складає близько %total-size% даних
* зареєстровано близько %total-users% користувачів, які залишили більше %total-comments% повідомлень

### Розробка

Розробка і підтримка проекту ведеться в основному силами %developer% ([contributors](https://github.com/autowp/autowp/graphs/contributors))

Переклад сайту на французьку мову: %fr-translator%

Переклад сайту на китайську мову: %zh-translator%

Переклад сайту на білоруську мову: %be-translator%

Переклад сайту на бразильську португальську: %pt-br-translator%

Переклад сайту на українську мову: %uk-translator%

Сайт робить на [Zend Framework](http://framework.zend.com/), [jQuery](http://jquery.com/), [Twitter bootstrap](http://getbootstrap.com/), а також багатьох інших "розумних словах".

Вихідний код сайту є відкритим, щоби кожний бажаючий мав можливість впливати на сутність і якість проекту.

%github%

[![Build Status](https://travis-ci.org/autowp/autowp.svg?branch=master)](https://travis-ci.org/autowp/autowp)
[![Code Climate](https://codeclimate.com/github/autowp/autowp/badges/gpa.svg)](https://codeclimate.com/github/autowp/autowp)
[![Coverage Status](https://coveralls.io/repos/github/autowp/autowp/badge.svg?branch=master)](https://coveralls.io/github/autowp/autowp?branch=master)

### Підтримати проект

Ви можете підтримати наш проект [фінансово](/donate) або [морально](/feedback).
Прийняти участь в [перекладі сайту](https://github.com/autowp/autowp/tree/master/module/Application/language) на інші мови.',
  'page/1/name' => 'Головна сторінка',
  'page/2/name' => 'Головне меню',
  'page/10/name' => 'Бренд',
  'page/14/name' => '%BRAND_NAME% в хронологічному порядку',
  'page/15/name' => 'Останні зображення %BRAND_NAME%',
  'page/19/name' => 'Виробники',
  'page/20/name' => 'Тип виробників',
  'page/21/name' => 'Най-най',
  'page/22/name' => 'Категорії',
  'page/24/name' => 'Лімітовані і спеціальні серії автомобілів',
  'page/25/name' => 'Близнята',
  'page/27/name' => 'Технічні характеристики %TWINS_GROUP_NAME%',
  'page/28/name' => 'Всі зображення %TWINS_GROUP_NAME%',
  'page/29/name' => 'Додати зображення',
  'page/30/name' => 'Вибір бренду',
  'page/34/name' => 'Всі зображення %CAR_NAME%',
  'page/36/name' => 'Технічні характеристики %CAR_NAME%',
  'page/37/name' => 'Концепти і прототипи %BRAND_NAME%',
  'page/39/name' => 'Логотипи %BRAND_NAME%',
  'page/40/name' => '%BRAND_NAME% різне',
  'page/41/name' => 'Несортовано',
  'page/45/name' => 'Додати топік',
  'page/48/name' => 'Особистий кабінет',
  'page/49/name' => 'Особисті повідомлення',
  'page/52/name' => 'Реєстрація',
  'page/53/name' => 'Успішна реєстрація',
  'page/54/name' => 'Підтвердження e-mail',
  'page/55/name' => 'Мій e-mail',
  'page/56/name' => 'Змінений',
  'page/57/name' => 'Підписки в форумі',
  'page/60/name' => 'Відновлення пароля',
  'page/61/name' => 'Всі виробники',
  'page/63/name' => 'Зображення користувача',
  'page/66/name' => 'Всі зображення %BRAND_NAME% %DESIGN_PROJECT_NAME%',
  'page/67/name' => 'Сторінка модератора',
  'page/68/name' => 'Сторінки сайту',
  'page/69/name' => 'Додати',
  'page/70/name' => 'Змінити',
  'page/71/name' => 'Права',
  'page/73/name' => 'Зображення',
  'page/74/name' => 'Автомобілі по алфавіту',
  'page/75/name' => 'Журнал подій',
  'page/76/name' => 'Немодероване',
  'page/77/name' => 'Трафік',
  'page/79/name' => 'Авторизація',
  'page/80/name' => 'Надіслані',
  'page/81/name' => 'Системні сповіщення',
  'page/83/name' => 'Переміщення',
  'page/87/name' => 'Іще',
  'page/89/name' => 'Зворотній зв\'язок',
  'page/90/name' => 'Вихід',
  'page/91/name' => 'Реєстрація',
  'page/93/name' => 'Повідомлення відправлене',
  'page/94/name' => 'Немодеровані',
  'page/97/name' => 'Ракурси',
  'page/100/name' => 'Атрибути',
  'page/102/name' => 'Редактор характеристик %CAR_NAME%',
  'page/103/name' => 'Історія змін',
  'page/104/name' => 'Статистика користувачів',
  'page/105/name' => 'Додати коментар',
  'page/107/name' => 'Заявки на видалення',
  'page/109/name' => 'Cutaway',
  'page/110/name' => 'Коментарі',
  'page/114/name' => 'Журнал ТТХ',
  'page/117/name' => 'Карта',
  'page/119/name' => 'Статистика',
  'page/120/name' => 'Блоки',
  'page/122/name' => 'Технічні характеристики',
  'page/123/name' => 'Мої облікові записи',
  'page/124/name' => 'Хто онлайн?',
  'page/128/name' => 'Вхідні',
  'page/129/name' => 'Профіль',
  'page/130/name' => 'Мої зображення',
  'page/131/name' => 'Одиниці даних',
  'page/133/name' => 'Керування доступом',
  'page/134/name' => 'Новий пароль',
  'page/135/name' => 'Новий пароль збережений',
  'page/137/name' => 'Видалення облікового запису',
  'page/138/name' => '%CAR_TYPE_NAME% %BRAND_NAME% в хронологічному порядку',
  'page/141/name' => 'Додані зображення %BRAND_NAME%',
  'page/144/name' => 'Вибір parent',
  'page/148/name' => 'Обрізка',
  'page/149/name' => 'Переміщення зображень',
  'page/153/name' => 'Близнята %BRAND_NAME%',
  'page/154/name' => 'Най %MOST_NAME%',
  'page/155/name' => 'Най %MOST_NAME% серед %CAR_TYPE_NAME%',
  'page/156/name' => 'Най %MOST_NAME% серед %CAR_TYPE_NAME% %YEAR_NAME%',
  'page/161/name' => 'Пульс',
  'page/162/name' => 'Всі зображення',
  'page/164/name' => 'Най-най',
  'page/166/name' => 'Най %MOST_NAME% серед %CAR_TYPE_NAME% %BRAND_NAME%',
  'page/167/name' => 'Най %MOST_NAME% серед %CAR_TYPE_NAME% %BRAND_NAME% %YEAR_NAME%',
  'page/173/name' => 'Статистика',
  'page/174/name' => 'Specs',
  'page/180/name' => 'Заводи',
  'page/182/name' => 'Продукція',
  'page/186/name' => 'Всі зображення',
  'page/188/name' => 'Конфлікти',
  'page/189/name' => 'Надто багато конфліктів',
  'page/196/name' => 'Пожертва',
  'page/197/name' => 'Історія зміни тексту',
  'page/198/name' => 'Контакти',
  'page/1/title' => 'Енциклопедія автомобілів в картинках. WheelsAge.org',
  'page/14/title' => '%BRAND_NAME% в хронологічному порядку',
  'page/15/title' => 'Останні зображення %BRAND_NAME%',
  'page/19/title' => 'Виробники',
  'page/22/title' => 'Категорії',
  'page/25/title' => 'Автомобілі-близнята',
  'page/27/title' => 'Технічні характеристики %TWINS_GROUP_NAME%',
  'page/28/title' => 'Всі зображення %TWINS_GROUP_NAME%',
  'page/34/title' => 'Всі зображення %CAR_NAME%',
  'page/36/title' => 'Технічні характеристики %CAR_NAME%',
  'page/37/title' => 'Концепти і прототипи %BRAND_NAME%',
  'page/39/title' => 'Логотипи %BRAND_NAME%',
  'page/40/title' => '%BRAND_NAME% Різне',
  'page/41/title' => 'Несортовано',
  'page/48/title' => 'Особистий кабінет',
  'page/56/title' => 'Зміна e-mail',
  'page/57/title' => 'Підписки в форумі',
  'page/66/title' => 'Всі зображення %BRAND_NAME% %DESIGN_PROJECT_NAME%',
  'page/87/title' => 'Іще',
  'page/102/title' => 'Редактор характеристик %CAR_NAME%',
  'page/105/title' => 'Додати коментар',
  'page/107/title' => 'Заявки на видалення',
  'page/114/title' => 'Журнал ТТХ',
  'page/122/title' => 'Технічні характеристики',
  'page/130/title' => 'Мої зображення',
  'page/138/title' => '%CAR_TYPE_NAME% %BRAND_NAME% в хронологічному порядку',
  'page/153/title' => 'Близнята %BRAND_NAME%',
  'page/162/title' => 'Всі зображення',
  'page/164/title' => 'Най-най',
  'page/166/title' => 'Най %MOST_NAME% серед %CAR_TYPE_NAME% %BRAND_NAME%',
  'page/167/title' => 'Най %MOST_NAME% серед %CAR_TYPE_NAME% %BRAND_NAME% %YEAR_NAME%',
  'page/180/title' => 'Заводи',
  'page/186/title' => 'Всі зображення',
  'page/189/title' => 'Надто багато конфліктів',
  'page/14/breadcrumbs' => 'В хронологічному порядку',
  'page/15/breadcrumbs' => 'Останні зображення',
  'page/19/breadcrumbs' => 'Виробники',
  'page/27/breadcrumbs' => 'Технічні характеристики',
  'page/28/breadcrumbs' => 'Всі зображення',
  'page/34/breadcrumbs' => 'Всі зображення',
  'page/36/breadcrumbs' => 'Specifications',
  'page/37/breadcrumbs' => 'Концепти і прототипи',
  'page/39/breadcrumbs' => 'Логотипи',
  'page/40/breadcrumbs' => 'Різне',
  'page/41/breadcrumbs' => 'Несортовано',
  'page/66/breadcrumbs' => 'Всі зображення',
  'page/102/breadcrumbs' => 'Редактор характеристик автомобіля',
  'page/122/breadcrumbs' => 'Технічні характеристики',
  'page/162/breadcrumbs' => 'Всі зображення',
  'page/164/breadcrumbs' => 'Най-най',
  'page/180/breadcrumbs' => 'Заводи',
  'page/186/breadcrumbs' => 'Всі зображення',
  'page/201/name' => 'Маскоти',
  'page/189/breadcrumbs' => 'Надто багато конфліктів',
  'page/202/name' => 'Довідка по ракурсам',
  'page/203/name' => 'Користувачі',
  'page/204/name' => 'Telegram',
  'page/205/name' => 'Коментарі',
  'page/208/name' => 'Двигуни %BRAND_NAME%',
  'page/208/title' => 'Двигуни %BRAND_NAME%',
  'page/208/breadcrumbs' => 'Двигуни %BRAND_NAME%',
  'page/211/name' => 'Контакти',
  'page/211/title' => 'Контакти',
  'page/211/breadcrumbs' => 'Контакти',
  'page/212/name' => 'Шаблони модераторських голосів',
  'moder-menu/title' => 'Меню модератора',
  'moder-menu/inbox' => 'Інбокс',
  'moder/database-id-%s' => 'В базі даних числиться під номером %s',
  'ng/moder/database-id-n' => 'В базі даних числиться під номером {id}',
  'moder/log-of-events' => 'Журнал подій',
  'moder/picture/inbox' => 'Інбокс',
  'moder/picture/missing-perspective' => 'Пропущений ракурс',
  'moder/picture/delete-queue' => 'Черга видалення',
  'moder/picture/votes' => 'Голоси',
  'moder/picture/comments' => 'Зафлудили',
  'moder/picture/new-votes' => 'Нові заявки',
  'moder/picture/replaces' => 'Нові заміни',
  'moder/picture/edit/special-name' => 'Особлива назва',
  'moder/picture/filter/status' => 'Статус',
  'moder/picture/filter/status/any' => 'будь-який',
  'moder/picture/filter/status/inbox' => 'інбокс',
  'moder/picture/filter/status/accepted' => 'прийнятий',
  'moder/picture/filter/status/removing' => 'в черзі на видалення',
  'moder/picture/filter/status/all-except-removing' => 'всі, крім видалених',
  'moder/picture/filter/vehicle-type' => 'Тип кузова',
  'moder/picture/filter/item' => 'Одиниця даних',
  'moder/picture/filter/owner' => 'Додав',
  'moder/picture/filter/special-name' => 'Тільки з особливою назвою',
  'moder/picture/filter/not-linked' => 'Без прив\'язки',
  'moder/picture/filter/gps' => 'З координатами',
  'moder/picture/filter/order' => 'Сортувати за',
  'moder/picture/filter/similar' => 'Подібні',
  'moder/pictures/filter' => 'Фільтр',
  'moder/pictures/filter/perspective' => 'Ракурс',
  'moder/pictures/filter/perspective/any' => 'будь-який',
  'moder/pictures/filter/perspective/empty' => 'не заданий',
  'moder/pictures/filter/comments' => 'Коментарі',
  'moder/pictures/filter/comments/not-matters' => 'не важливо',
  'moder/pictures/filter/comments/has-comments' => 'є',
  'moder/pictures/filter/comments/has-no-comments' => 'нема',
  'moder/pictures/filter/replace' => 'Заміна',
  'moder/pictures/filter/replace/not-matters' => 'не важливо',
  'moder/pictures/filter/replace/replaces' => 'заміна',
  'moder/pictures/filter/replace/without-replaces' => 'крім замін',
  'moder/pictures/filter/votes' => 'Заявки на приняття/видалення',
  'moder/pictures/filter/votes/not-matters' => 'не важливо',
  'moder/pictures/filter/votes/none' => 'нема',
  'moder/pictures/filter/votes/accept' => 'є на прийняття',
  'moder/pictures/filter/votes/delete' => 'є на видалення',
  'moder/pictures/filter/votes/any' => 'є будь-які',
  'moder/pictures/filter/order/add-date-desc' => 'Дата додавання (нові)',
  'moder/pictures/filter/order/add-date-asc' => 'Дата додавання (старі)',
  'moder/pictures/filter/order/resolution-desc' => 'Розширення (великі)',
  'moder/pictures/filter/order/resolution-asc' => 'Розширення (маленькі)',
  'moder/pictures/filter/order/filesize-desc' => 'Розмір (великі)',
  'moder/pictures/filter/order/filesize-asc' => 'Розмір (маленькі)',
  'moder/pictures/filter/order/commented' => 'Коментуємі',
  'moder/pictures/filter/order/views' => 'Перегляди',
  'moder/pictures/filter/order/moder-votes' => 'Заявки на прийняття/видалення',
  'moder/pictures/filter/order/removing-date' => 'Дата видалення',
  'moder/pictures/filter/order/likes' => 'Лайки',
  'moder/pictures/filter/order/dislikes' => 'Дизлайки',
  'moder/pictures/acceptance/message' => 'Повідомлення',
  'moder/items/filter' => 'Фільтр',
  'moder/items/filter/name' => 'Назва',
  'moder/items/filter/name-exclude' => 'Назва (виключити)',
  'moder/items/filter/item-type' => 'Тип',
  'moder/items/filter/vehicle-type' => 'Тип кузова',
  'moder/items/filter/vehicle-type/empty' => 'Не заповнено',
  'moder/items/filter/spec' => 'Ринок',
  'moder/items/filter/from-year' => 'Рік початку',
  'moder/items/filter/to-year' => 'Рік кінця',
  'moder/items/filter/parent' => 'Батько',
  'moder/items/filter/category-exclude' => 'Категорія (виключити)',
  'moder/items/filter/no-parents' => 'Без батьків',
  'moder/items/filter/order' => 'Сортування',
  'moder/items/filter/text' => 'Текст',
  'moder/vehicle/name' => 'Назва',
  'moder/vehicle/body' => 'Номер кузову',
  'moder/vehicle/spec' => 'Ринок',
  'moder/vehicle/type' => 'Тип кузова',
  'moder/vehicle/model-years' => 'Модельні роки',
  'moder/vehicle/begin' => 'Початок',
  'moder/vehicle/end' => 'Кінець',
  'moder/item/produced' => 'Виготовлено',
  'moder/item/produced/number' => 'одиниць',
  'moder/item/produced/precision' => 'точно?',
  'moder/item/produced/about' => 'приблизно',
  'moder/item/produced/exactly' => 'точно',
  'moder/vehicle/concept' => 'Концепт (прототип)',
  'moder/vehicle/group' => 'Група',
  'moder/vehicle/year' => 'рік',
  'moder/vehicle/year/from' => 'з',
  'moder/vehicle/year/to' => 'по',
  'moder/vehicle/month' => 'місяць',
  'moder/vehicle/today' => 'наш час',
  'moder/vehicle/today/ended' => 'випуск закінчено',
  'moder/vehicle/today/continue' => 'виробляється в н.ч.',
  'moder/vehicle/is-concept/no' => 'ні',
  'moder/vehicle/is-concept/yes' => 'так',
  'moder/vehicle/is-concept/inherited-no' => 'успадковано (ні)',
  'moder/vehicle/is-concept/inherited-yes' => 'успадковано (так)',
  'moder/vehicle/is-concept/inherited' => 'успадковано',
  'moder/item/short-description' => 'короткий опис',
  'moder/item/full-description' => 'Повний опис',
  'moder/vehicle/tabs/meta' => 'Мета',
  'moder/vehicle/tabs/name' => 'Назва та опис',
  'moder/vehicle/tabs/catalogue' => 'Каталог',
  'moder/vehicle/tabs/tree' => 'Дерево',
  'moder/vehicle/tabs/categories' => 'Категорії',
  'moder/vehicle/tabs/twins' => 'Близнята',
  'moder/vehicle/tabs/factories' => 'Заводи',
  'moder/vehicle/tabs/pictures' => 'Зображення',
  'moder/vehicle/tabs/modifications' => 'Модифікації',
  'moder/vehicle/tabs/vehicles' => 'ТЗ',
  'moder/vehicle/meta/description' => 'короткий опис',
  'moder/vehicle/subscription/title' => 'Підписка на повідомлення про зміни',
  'moder/vehicle/subscription/subscribed' => 'Оформлена',
  'moder/vehicle/subscription/not-subscribed' => 'Не оформлена',
  'moder/vehicle/catalogue/add-vehicle' => 'додати автомобіль',
  'moder/vehicle/catalogue/add-engine' => 'додати двигун',
  'moder/vehicle/catalogue/add-category' => 'додати категорію',
  'moder/vehicle/catalogue/title/name' => 'Назва',
  'moder/vehicle/catalogue/title/link' => 'Зв\'язок',
  'moder/vehicle/catalogue/title/catname' => 'Каталог',
  'moder/vehicle/catalogue/title/public' => 'На сайті',
  'moder/vehicle/catalogue/title/remove' => 'Вид.',
  'moder/vehicle/catalogue/select' => 'Вибрати ...',
  'moder/vehicle/catalogue/select-placeholder' => 'Додати батька ...',
  'moder/vehicle/catalogue/possible-duplicate-parent' => 'Можливо повтор: автомобіль уже унаслідуваний через: ',
  'moder/vehicle/catalogue/possible-duplicate-child' => 'Можливо повтор: автомобіль вже є у в підрозділі: ',
  'moder/vehicle/catalogue/parent-name-placeholder' => 'Назва',
  'moder/vehicle/catalogue/parent-catname-placeholder' => 'частина URL',
  'moder/vehicle/catalogue/turbo-group-creator' => 'Турбо-об\'єднювач в групи',
  'moder/vehicle/catalogue/turbo-group-creator/text' => 'Інструмент для об\'єднання декількох автомобілей в під-модель',
  'moder/vehicle/catalogue/turbo-group-creator/vehicles' => 'Автомобілі',
  'moder/vehicle/catalogue/add-brand' => 'Бренд ...',
  'moder/vehicle/catalogue/add-brand-%s' => 'Бренд %s',
  'ng/moder/vehicle/catalogue/add-brand-{brand}' => 'Бренд {brand}',
  'moder/vehicle/twins/select-or-create' => 'Обрати або створити групу ...',
  'moder/vehicle/twins/group-not-selected' => 'Група не задана',
  'moder/vehicle/twins/remove' => 'Видалити',
  'moder/vehicle/twins/inherited-from' => 'Успадковано від',
  'moder/vehicle/factories/select-factory' => 'Вибрати завод ...',
  'moder/vehicle/factories/factory-not-selected' => 'Завод не заданий',
  'moder/vehicle/factories/remove' => 'Видалити',
  'moder/vehicle/factories/inherited-from' => 'Успадковано від',
  'moder/vehicle/link-type-prefix/design' => 'Дизайн: ',
  'moder/vehicle/link-type-prefix/sport' => 'Спорт: ',
  'moder/vehicle/link-type-prefix/related' => 'Пов\'язані: ',
  'moder/vehicle/move/here' => 'сюди',
  'moder/vehicle/pictures/turbo-group-creator' => 'Турбо-об\'єднювач картинок в групи',
  'moder/vehicle/pictures/turbo-group-creator/text' => 'Інструмент дозволяє об\'єднати кілька картинок у автомобіль',
  'moder/vehicle/pictures/turbo-group-creator/pictures' => 'Зображення',
  'moder/vehicle/add/as-submodel' => 'В автомобіль',
  'attrs/attribute/name' => 'Назва',
  'attrs/attribute/type' => 'Тип',
  'attrs/attribute/unit' => 'Одиниця виміру',
  'attrs/attribute/precision' => 'Точність (для float атрибута)',
  'attrs/attribute/description' => 'Опис',
  'attrs/list-options/parent' => 'Батько',
  'attrs/list-options/name' => 'Назва',
  'moder/attrs/zones' => 'Зони',
  'moder/attrs/parameters' => 'Параметри атрибута',
  'moder/attrs/parameters/options-list' => 'Список значень (для спискових типів)',
  'moder/attrs/parameters/options-list/add' => 'Додати значення',
  'moder/attrs/attributes' => 'Атрибути',
  'moder/attrs/attribute/add-subattribute' => 'Додати підатрибут',
  'moder/attrs/attributes-order' => 'Порядок атрибутів',
  'category/name' => 'Назва',
  'category/parent' => 'Батько',
  'categories/other' => 'Інші',
  'moder/categories/add' => 'Додати',
  'moder/categories/edit' => 'Змінити',
  'moder/categories/new' => 'Новий',
  'moder/picture/edit-picture-%s' => 'Управління зображенням №%s',
  'moder/picture/edit-item-%s' => 'Управління %s',
  'moder/picture/edit-brand-%s' => 'Управління брендом %s',
  'moder/picture/edit-engine-%s' => 'Управління двигуном %s',
  'moder/picture/edit-factory-%s' => 'Управління заводом %s',
  'moder/picture/picture-n' => 'Зображення №{id}',
  'moder/picture/previous' => '<< попереднє',
  'moder/picture/next' => 'наступне >>',
  'moder/picture/previous-new' => '<< попереднє не оброблене',
  'moder/picture/next-new' => 'наступне не оброблене >>',
  'moder/picture/catalogue' => 'Місце в каталозі',
  'moder/picture/catalogue/vehicle' => 'Автомобіль: ',
  'moder/picture/catalogue/logo' => 'Логотип бренду: ',
  'moder/picture/catalogue/mixed' => 'Змішане бренду:',
  'moder/picture/catalogue/unsorted' => 'Несортовано бренду:',
  'moder/picture/catalogue/engine' => 'Двигун: ',
  'moder/picture/catalogue/factory' => 'Завод: ',
  'moder/picture/catalogue/not-set' => 'не заданий',
  'moder/picture/catalogue/move-to' => 'перемістити у',
  'moder/picture/catalogue/add-to' => 'додати у',
  'moder/picture/catalogue/pick-item-area' => 'Відмітити область',
  'moder/picture/perspective' => 'Ракурс',
  'moder/picture/perspective-by' => 'Встановив:',
  'moder/picture/acceptance' => 'Статус/Видалення/Прийняття',
  'moder/picture/acceptance/accepted' => 'Прийнято',
  'moder/picture/acceptance/not-accepted' => 'Не прийнято',
  'moder/picture/acceptance/in-delete-queue' => 'Поставлено в чергу на видалення',
  'moder/picture/acceptance/accept' => 'Прийняти',
  'moder/picture/acceptance/delete' => 'Видалити',
  'moder/picture/acceptance/unaccept' => 'Зробити не прийнятою',
  'moder/picture/acceptance/restore' => 'Відновити',
  'moder/picture/acceptance/removed' => 'Видалено',
  'moder/picture/acceptance/removing' => 'Видаляєтья',
  'moder/picture/acceptance/inbox' => 'Вхідні',
  'moder/picture/acceptance/reason' => 'Причина',
  'moder/picture/acceptance/custom' => 'Своя ...',
  'moder/picture/acceptance/add-reason' => 'Додати шаблон',
  'moder/picture/acceptance/vote' => 'Голос',
  'moder/picture/acceptance/want-accept' => 'Хочу прийняти',
  'moder/picture/acceptance/want-delete' => 'Хочу видалити',
  'moder/picture/acceptance/cancel-vote' => 'Відмінити мою заявку',
  'moder/picture/acceptance/already-voted' => 'Вже подали:',
  'moder/picture/acceptance/that-is-one-accepted-picture' => 'Це єдине прийняте зображення цього автомобіля',
  'ng/moder/picture/acceptance/accepted-pictures-is-n' => 'Прийнятих зображень цього автомобіля всього {count}',
  'moder/picture/acceptance/accepted-pictures-is-%s' => 'Прийнятих зображень цього автомобіля всього %s',
  'moder/picture/public-url:' => 'На сайті:',
  'moder/picture/image:' => 'Зображення:',
  'moder/picture/image-specs:' => 'Характеристики зображення:',
  'moder/picture/resolution:' => 'Роздільна здатність:',
  'moder/picture/filesize:' => 'Розмір файлу:',
  'moder/picture/upload-date:' => 'Додано:',
  'moder/picture/settings' => 'Налаштування',
  'moder/picture/copyrights' => 'Копірайти',
  'moder/picture/replacement' => 'Заміна',
  'moder/picture/replacement/photo-suggested-to-replace' => 'Фото запропоноване на заміну',
  'moder/picture/replacement/accept-and-delete-double' => 'Прийняти заміну і видалити дубль',
  'moder/picture/replacement/cancel' => 'Відмінити заміну',
  'moder/picture/edit-and-repair' => 'Редагування і усунення проблем',
  'moder/picture/edit-and-repair/normalize' => 'Автоконтраст',
  'moder/picture/edit-and-repair/flop' => 'Відобразити',
  'moder/picture/edit-and-repair/crop' => 'Область обрізання',
  'moder/picture/edit-and-repair/repair-all-files' => 'Виправити всі файли',
  'moder/picture/edit-and-repair/rebuild-files-names' => 'Перерахувати імена файлів',
  'moder/picture/uploader' => 'Завантажив',
  'moder/picture/uploader/unknown' => 'Невідомо',
  'moder/picture/uploader/id-address:' => 'IP-адреса завантажувача:',
  'moder/pciture/move/brands' => 'Бренди',
  'moder/pciture/move/factories' => 'Заводи',
  'moder/pciture/move/engines' => 'Двигуни',
  'brand' => 'Бренд',
  'brand/name' => 'Назва',
  'brand/logo' => 'Логотип',
  'moder/brands/meta-data/full-name' => 'Повна назва',
  'moder/brands/logo/description' => '* Логотип повинен бути строго в форматі PNG.
* Всі прозорі області повинні бути прозорими, а не білими!
* Логотип повинен торкатися країв зображень. Ніяких полів непотрібно
* Завантажуйте в максимально доступній роздільній здатності - це забезпечить якість і можливість в майбутньому десь скористуватися цим великим фото. На сайті всі розміри будуть підігнані',
  'moder/brands/logo/saved' => 'Логотип збережений',
  'moder/brands/links' => 'Посилання',
  'moder/brands/links/text' => 'Текст',
  'moder/brands/links/address' => 'Адреса',
  'moder/brands/links/type' => 'Тип',
  'moder/brands/links/type/interest-link' => 'цікаве посилання',
  'moder/brands/links/type/official' => 'офіційний сайт',
  'moder/brands/links/type/club' => 'клуб',
  'moder/edit-object' => 'управління',
  'moder/markdown/description' => 'Синтаксис Markdown.

Декілька описів мови:
[1](https://ru.wikipedia.org/wiki/Markdown),
[2](https://en.wikipedia.org/wiki/Markdown),
[3](https://guides.github.com/features/mastering-markdown/),
[4](https://daringfireball.net/projects/markdown/basics),
[5](http://rukeba.com/by-the-way/markdown-sintaksis-po-russki)

Посилання на внутрішньосайтові сторінки потрібно писати не повністю: ~~http://autowp.ru/bmw/~~ */bmw/*',
  'moder/markdown/edit' => 'Редактувати',
  'moder/markdown/preview' => 'Попередній перегляд',
  'moder/markdown/history' => 'Історія',
  'moder/users/login' => 'Логін',
  'moder/users/name' => 'Ім\'я',
  'moder/users/role' => 'Роль',
  'moder/users/profile' => 'Профіль',
  'moder/users/last-visit' => 'Останній вхід',
  'moder/users/registration' => 'Реєстрація',
  'moder/twins/public-url:' => 'На сайті:',
  'moder/twins/meta-data' => 'Мета-інформація',
  'moder/twins/name' => 'Назва',
  'moder/twins/short-description' => 'Короткий опис',
  'moder/twins/vehicles' => 'Автомобілі',
  'moder/twins/add/title' => 'Створення нової групи близнят',
  'latitude' => 'Широта',
  'longtitude' => 'Довгота',
  'museum/name' => 'Назва',
  'museum/address' => 'Адреса',
  'museum/photo' => 'Фотографія',
  'museum/description' => 'Опис',
  'moder/museums/add' => 'Додати музей',
  'factory/name' => 'Назва',
  'factory/year_from' => 'Рік з',
  'factory/year_to' => 'Рік по',
  'moder/factories/title' => 'Заводи',
  'moder/factories/filter' => 'Фільтр',
  'moder/factories/add' => 'додати завод',
  'moder/factories/add/title' => 'Завод',
  'moder/factories/edit/title' => 'Завод',
  'moder/factories/puclic-url:' => 'На сайті:',
  'moder/factories/factory/short-description' => 'Короткий опис',
  'moder/factories/factory/vehicles' => 'Автомобілі заводу',
  'moder/factories/filter/brand-id' => 'Бренд №',
  'moder/factories/filter/order' => 'Сортування',
  'moder/factories/filter/order/name-asc' => 'Назва за зростанням',
  'moder/factories/filter/order/name-desc' => 'Назва за спаданням',
  'moder/comments/title' => 'Коментарі',
  'moder/comments/filter' => 'Фільтр',
  'moder/comments/filter/user-id' => 'Користувач №',
  'moder/comments/filter/brand-id' => 'Бренд №',
  'moder/comments/filter/moderator_attention' => 'Увага модераторів',
  'moder/comments/filter/moderator_attention/not-matters' => 'Не важливо',
  'moder/comments/filter/moderator_attention/not-required' => 'Не потрібно',
  'moder/comments/filter/moderator_attention/required' => 'Потрібно',
  'moder/comments/filter/moderator_attention/resolved' => 'Виповнено',
  'moder/comments/filter/vehicle-id' => 'Одиниця даних',
  'moder/comments/not-readed' => 'не прочитано',
  'engine/name' => 'Назва',
  'moder/engines/engine/vehicles' => 'Автомобіль на цьому двигуні',
  'page/name' => 'Назва',
  'page/is_group_node' => 'Групостворюючий вузол?',
  'page/registered_only' => 'Тільки для зареєстрованих?',
  'page/guests_only' => 'Тільки для гостей?',
  'page/class' => 'Клас',
  'page/parent' => 'Батько',
  'moder/pages/new' => 'Новий',
  'moder/index/other-tools' => 'Інші інструменти',
  'moder/acl/add-role' => 'Додати роль',
  'moder/acl/add-rule' => 'Додати правило',
  'moder/acl/add-rule/action' => 'Дія',
  'moder/acl/add-rule/action/allow' => 'дозволити',
  'moder/acl/add-rule/action/deny' => 'заборонити',
  'moder/acl/add-parent' => 'Додати батька',
  'moder/acl/role' => 'Роль',
  'moder/acl/parent-role' => 'Батьківська роль',
  'moder/acl/privilege' => 'Привілегія',
  'votings/do-vote' => 'Голосувати',
  'votings/who-voted' => 'Хто проголосував?',
  'votings/voting/duration' => 'Опитування проводиться з {from} по {to}',
  'votings/voting/voters/show-all' => 'показати всіх',
  'votings/voting/voters/show-contributors' => 'показати тільки від 100 завантажених зображень',
  'museums/museum/address:' => 'Адреса:',
  'museums/museum/on-the-map' => 'на карті',
  'museums/museum/website:' => 'Web-сайт: ',
  'restore-password/text' => 'Якщо ви забули свій пароль - введіть ваш e-mail (який ви вказали при реєстрації) і на нього буде надісланий пароль.',
  'restore-password/new-password/text' => 'Введіть новий пароль',
  'restore-password/new-password/saved' => 'Новий пароль успішно збережений.

Не забувайте його.',
  'restore-password/new-password/instructions-sent' => 'На ваш e-mail відправлені подальші інструкції',
  'restore-password/new-password/email-not-found' => 'Користувач з таким e-mail не знайдений',
  'restore-password/new-password/mail/subject' => 'Відновлення пароля',
  'restore-password/new-password/mail/body-%s' => 'Для вводу нового паролю перейдіть за посиланням: %s

З Повагою, робот www.wheelsage.org
',
  'twins/group/name' => 'Назва',
  'twins/group/description' => 'Опис',
  'specifications-editor/description' => 'Прохання віднестися з усією відповідальністю до усіх виконуваних дій, оскільки вони стануть надбанням громадськості.


Про структуру самих характеристик.

Всі характеристики організовані у вигляді дерева елементів "параметр - значення". Значення можуть бути числовими і текстовими.

Важливою особливістю є той факт, що кожна характеристика може бути задана одночасно декількома користувачами. В цьому випадку актуальним (використовуваним) значенням є те, яке система визнає пріоритетним по ряду ознак.

Якщо ви зіткнулися з ситуацією, коли вам не вдається перекрити введене кимось раніше значення, то ви можете просто написати про це авторові. Втім, в процесі реалізації, система автоматично повідомить авторові про те, що з його значенням хтось не згоден.


Всі значення, які тут вводяться - відображаються на сайті, в характеристиках, використовуються в побудові розділу "най-най", а також відображаються в деяких інших місцях сайту.',
  'specifications-editor/not-save' => 'Дані не збережені через помилку. Подробиці нижче',
  'specifications-editor/parameter' => 'Характеристика',
  'specifications-editor/your-value' => 'Ваше значення',
  'specifications-editor/actual-value' => 'Актуальне значення',
  'specifications-editor/all-values' => 'Введені значення',
  'specifications-editor/tabs/info' => 'Інформація',
  'specifications-editor/tabs/engine' => 'Двигун',
  'specifications-editor/tabs/specs' => 'Основні ТТХ',
  'specifications-editor/tabs/result' => 'Результат',
  'specifications-editor/tabs/admin' => 'Admin',
  'specifications-editor/engine' => 'Двигун з каталогу',
  'specifications-editor/engine/inherited-from' => 'Успадковано від',
  'specifications-editor/engine/select-another' => 'Вибрати інший двигун',
  'specifications-editor/engine/cancel' => 'Відмітити двигун',
  'specifications-editor/engine/inherit' => 'Наслідувати двигун',
  'specifications-editor/engine/not-selected' => '[не вибраний]',
  'specifications-editor/engine/select' => 'Вибрати двигун',
  'specifications-editor/engine/dont-inherit' => 'Не наслідувати двигун',
  'specifications-editor/save' => 'Зберегти',
  'specifications-editor/log' => 'Журнал ТТХ',
  'specifications-editor/log/date' => 'Дата',
  'specifications-editor/log/user' => 'Користувач',
  'specifications-editor/log/object' => 'Об\'єкт',
  'specifications-editor/log/attribute' => 'Атрибут',
  'specifications-editor/log/value' => 'Значення',
  'specifications-editor/log/editor' => 'Редактор',
  'specifications-editor/log/to-editor' => 'в редактор',
  'specifications-editor/log/filter/user-id' => 'Користувач',
  'specifications-editor/log/low-weight-text' => 'Зареєстровано велику кількість конфліктів в даних, введених вами. Інші люди часто спростовують інформацію, надану вами.

Введення даних тимчасово призупинено. Прохання виправити ситуацію, провести ревізію конфліктів.

Здійснити це допоможе [спеціальний інтерфейс](/account/specs-conflicts/conflict/minus-weight), в якому зібрані як мінімум ті помилки, які помітили користувачі сайту.',
  'specifications-editor/errors-alert' => '**Увага!** Імовірно, у вас накопичилася велика кількість помилок.

Деякі з них, помічені іншими користувачами, ви можете знайти [тут](/account/specs-conflicts)',
  'specifications-editor/admin/date' => 'Дата',
  'specifications-editor/admin/user' => 'Користувач',
  'specifications-editor/admin/parameter' => 'Характеристика',
  'specifications-editor/admin/value' => 'Значення',
  'specifications-editor/admin/move' => 'Перенести',
  'pm/user-%s-edited-brand-description-%s-%s' => 'Користувач %s редактував опис бренду %s ( %s )',
  'pm/user-%s-edited-item-language-%s-%s' => 'Користувач %s редактував мовну інформацію %s ( %s )
%s',
  'pm/user-%s-edited-vehicle-meta-data-%s-%s-%s' => 'Користувач %s редактував інформацію про %s ( %s )
%s',
  'pm/user-%s-adds-item-%s-%s-to-item-%s-%s' => 'Користувач %s додав %s ( %s ) в %s ( %s )',
  'pm/user-%s-removed-item-%s-%s-from-item-%s-%s' => 'Користувач %s видалив %s ( %s ) з %s ( %s )',
  'pm/user-%s-cancel-link-vehicle-%s-%s-with-categories-%s' => 'Користувач %s відмінив вашу прив\'язку автомобіля %s ( %s ) до категорій: %s',
  'pm/user-%s-edited-factory-description-%s-%s' => 'Користувач %s редактував опис заводу %s ( %s )',
  'pm/your-picture-%s-enqueued-to-remove-%s' => 'Додане вами зображення %s поставлене в чергу на видалення
%s',
  'pm/new-picture-%s-vote-%s/accept' => 'Подана заява на прийняття доданого вами зображення
%s
Причина: %s',
  'pm/new-picture-%s-vote-%s/delete' => 'Подана заява на видалення доданого вами зображення
%s
Причина: %s',
  'pm/user-%s-edited-picture-copyrights-%s-%s' => 'Користувач %s редактував текст копірайтів зображення %s ( %s )',
  'pm/user-%s-accept-replace-%s-%s' => '%s прийняв заміну %s на %s',
  'pm/your-picture-accepted-%s' => 'Додане вами зображення прийняте на сайт
%s',
  'pm/user-%s-edited-twins-description-%s-%s' => 'Користувач %s редактував опис групи близнят %s ( %s )',
  'pm/user-%s-edited-vehicle-specs-%s' => '%s вніс ТТХ для автомобіля %s',
  'pm/user-%s-canceled-vehicle-engine-%s-%s-%s' => '%s відмінив двигун %s для автомобіля %s ( %s )',
  'pm/user-%s-set-inherited-vehicle-engine-%s-%s' => '%s встановив успадкування двигуна автомобілю %s ( %s )',
  'pm/user-%s-set-vehicle-engine-%s-%s-%s' => '%s назначив двигун %s автомобілю %s ( %s )',
  'pm/user-%s-replies-to-you-%s' => '%s відповів на ваше повідомлення
%s',
  'pm/user-%s-post-new-message-%s' => '%s написав повідомлення
%s',
  'moder/vehicle/changes/name-%s-%s' => 'назву з "%s" на "%s"',
  'moder/vehicle/changes/body-%s-%s' => 'номер кузову з "%s" на "%s"',
  'moder/vehicle/changes/from/year-%s-%s' => 'рік початку випуску з "%s" на "%s"',
  'moder/vehicle/changes/from/month-%s-%s' => 'місяць початку випуску з "%s" на "%s"',
  'moder/vehicle/changes/to/year-%s-%s' => 'рік припинення випуску з "%s" на "%s"',
  'moder/vehicle/changes/to/month-%s-%s' => 'місяць закінчення випуску з "%s" на "%s"',
  'moder/vehicle/changes/to/today-%s-%s' => 'випуск в наш час з "%s" на "%s"',
  'moder/vehicle/changes/produced/count-%s-%s' => 'кількість випущених одиниць з "%s" на "%s"',
  'moder/vehicle/changes/produced/exactly-%s-%s' => 'точність кількості випущених одиниць з "%s" на "%s"',
  'moder/vehicle/changes/is-group-%s-%s' => 'прапор "група" з "%s" на "%s"',
  'moder/vehicle/changes/car-type-%s-%s' => 'тип кузова з "%s" на "%s"',
  'moder/vehicle/changes/model-years/from-%s-%s' => 'модельний рік початку випуску з "%s" на "%s"',
  'moder/vehicle/changes/model-years/to-%s-%s' => 'модельний рік закінчення випуску з "%s" на "%s"',
  'moder/vehicle/changes/spec-%s-%s' => 'spec з "%s" на "%s"',
  'moder/vehicle/changes/boolean/true' => 'так',
  'moder/vehicle/changes/boolean/false' => 'ні',
  'specifications/no-value-text' => 'нема значення',
  'specifications/boolean/false' => 'ні',
  'specifications/boolean/true' => 'так',
  'Brand \'%value%\' already exists' => 'Бренд з назвою \'%value%\' вже існує',
  'E-mail \'%value%\' not registered' => 'E-mail \'%value%\' не зареєстрований на сайті',
  'E-mail \'%value%\' already registered' => 'E-mail \'%value%\' вже зареєстрований на сайті',
  'specs/attrs/45' => 'Назва модифікації',
  'specs/attrs/95' => 'Роки випуску',
  'specs/attrs/95/96' => 'з',
  'specs/attrs/95/97' => 'по',
  'specs/attrs/95/106' => 'Продажі',
  'specs/attrs/95/106/109' => 'з',
  'specs/attrs/95/106/109/129' => 'рік',
  'specs/attrs/95/106/109/130' => 'місяць',
  'specs/attrs/95/106/109/131' => 'число',
  'specs/attrs/95/106/111' => 'по',
  'specs/attrs/95/106/111/132' => 'рік',
  'specs/attrs/95/106/111/133' => 'місяць',
  'specs/attrs/95/106/111/134' => 'число',
  'specs/attrs/95/104' => 'модельні',
  'specs/attrs/95/104/113' => 'з',
  'specs/attrs/95/104/114' => 'по',
  'specs/attrs/95/107' => 'дебют на автошоу',
  'specs/attrs/95/107/118' => 'рік',
  'specs/attrs/95/107/119' => 'місяць',
  'specs/attrs/95/107/120' => 'число',
  'specs/attrs/95/108' => 'виробництво',
  'specs/attrs/95/108/121' => 'з',
  'specs/attrs/95/108/121/123' => 'рік',
  'specs/attrs/95/108/121/124' => 'місяць',
  'specs/attrs/95/108/121/125' => 'число',
  'specs/attrs/95/108/122' => 'по',
  'specs/attrs/95/108/122/126' => 'рік',
  'specs/attrs/95/108/122/127' => 'місяць',
  'specs/attrs/95/108/122/128' => 'число',
  'specs/attrs/95/135' => 'участь у змаганнях',
  'specs/attrs/95/135/136' => 'з',
  'specs/attrs/95/135/137' => 'по',
  'specs/attrs/16' => 'основні',
  'specs/attrs/16/12' => 'кількість місць',
  'specs/attrs/16/12/67' => 'всього',
  'specs/attrs/16/12/67/description' => 'сидячих',
  'specs/attrs/16/12/68' => 'з них неповноцінних',
  'specs/attrs/16/12/69' => 'стоячих',
  'specs/attrs/16/12/103' => 'загальна місткість',
  'specs/attrs/16/12/103/description' => 'з врахуванням стоячих місць',
  'specs/attrs/16/13' => 'кількість дверей',
  'specs/attrs/16/66' => 'розташування руля',
  'specs/attrs/16/66/options/11' => 'зліва',
  'specs/attrs/16/66/options/12' => 'зправа',
  'specs/attrs/16/66/options/13' => 'по центру',
  'specs/attrs/16/204' => 'конструкція',
  'specs/attrs/16/204/options/85' => 'несучий кузов',
  'specs/attrs/16/204/options/86' => 'рамна',
  'specs/attrs/14' => 'геометрія',
  'specs/attrs/14/4' => 'колісна база',
  'specs/attrs/14/17' => 'габарити',
  'specs/attrs/14/17/description' => 'Зовнішні розміри',
  'specs/attrs/14/17/1' => 'довжина',
  'specs/attrs/14/17/2' => 'ширина',
  'specs/attrs/14/17/3' => 'висота',
  'specs/attrs/14/17/140' => 'ширина, з врахуванням зеркал',
  'specs/attrs/14/17/141' => 'висота, з врахуванням рейлінгів',
  'specs/attrs/14/17/203' => 'висота з відкритим верхом',
  'specs/attrs/14/18' => 'колія',
  'specs/attrs/14/18/5' => 'передня',
  'specs/attrs/14/18/6' => 'задня',
  'specs/attrs/14/63' => 'аеродинамічний спротив',
  'specs/attrs/14/63/64' => 'лобовий',
  'specs/attrs/14/63/65' => 'боковий',
  'specs/attrs/14/167' => 'кліренс',
  'specs/attrs/14/167/description' => '',
  'specs/attrs/14/167/176' => 'мін',
  'specs/attrs/14/167/7' => 'стандарт',
  'specs/attrs/14/167/168' => 'макс',
  'specs/attrs/70' => 'вага',
  'specs/attrs/70/71' => 'суха',
  'specs/attrs/70/72' => 'споряджена',
  'specs/attrs/70/73' => 'повна',
  'specs/attrs/22' => 'двигун',
  'specs/attrs/22/100' => 'назва',
  'specs/attrs/22/98' => 'пальне',
  'specs/attrs/22/98/options/28' => 'бензин',
  'specs/attrs/22/98/options/29' => 'водень',
  'specs/attrs/22/98/options/30' => 'біоетанол',
  'specs/attrs/22/98/options/31' => 'електрика',
  'specs/attrs/22/98/options/32' => 'газ',
  'specs/attrs/22/98/options/33' => 'дизель',
  'specs/attrs/22/98/options/84' => 'Flex-fuel',
  'specs/attrs/22/98/options/36' => 'А-66',
  'specs/attrs/22/98/options/37' => 'А-70',
  'specs/attrs/22/98/options/38' => 'А-72',
  'specs/attrs/22/98/options/39' => 'А-76',
  'specs/attrs/22/98/options/40' => 'А-78',
  'specs/attrs/22/98/options/41' => 'А-80',
  'specs/attrs/22/98/options/42' => 'АІ-92',
  'specs/attrs/22/98/options/43' => 'АІ-93',
  'specs/attrs/22/98/options/44' => 'АІ-95',
  'specs/attrs/22/98/options/45' => 'АІ-98',
  'specs/attrs/22/98/options/34' => 'CNG',
  'specs/attrs/22/98/options/35' => 'LPG',
  'specs/attrs/22/19' => 'розположення',
  'specs/attrs/22/19/20' => 'розположення',
  'specs/attrs/22/19/20/options/1' => 'спереду',
  'specs/attrs/22/19/20/options/2' => 'ззаду',
  'specs/attrs/22/19/20/options/3' => 'центральне',
  'specs/attrs/22/19/21' => 'орієнтація',
  'specs/attrs/22/19/21/options/4' => 'поздовжньо',
  'specs/attrs/22/19/21/options/5' => 'поперечно',
  'specs/attrs/22/23' => 'система живлення',
  'specs/attrs/22/23/options/6' => 'інжектор',
  'specs/attrs/22/23/options/24' => 'карбюратор',
  'specs/attrs/22/23/options/25' => '2 карбюратори',
  'specs/attrs/22/23/options/55' => '3 карбюратори',
  'specs/attrs/22/23/options/26' => '4 карбюратори',
  'specs/attrs/22/23/options/27' => '6 карбюраторів',
  'specs/attrs/22/24' => 'циліндри/клапани',
  'specs/attrs/22/24/25' => 'кількість циліндрів',
  'specs/attrs/22/24/26' => 'розміщення циліндрів',
  'specs/attrs/22/24/26/options/7' => 'l',
  'specs/attrs/22/24/26/options/8' => 'V',
  'specs/attrs/22/24/26/options/9' => 'W',
  'specs/attrs/22/24/26/options/10' => 'O',
  'specs/attrs/22/24/26/options/101' => 'U',
  'specs/attrs/22/24/27' => 'клапанів на циліндр',
  'specs/attrs/22/24/28' => 'діаметр циліндрів',
  'specs/attrs/22/24/29' => 'хід поршня',
  'specs/attrs/22/24/159' => 'кут розвалу',
  'specs/attrs/22/30' => 'ступінь стиснення',
  'specs/attrs/22/31' => 'об\'єм',
  'specs/attrs/22/32' => 'потужність',
  'specs/attrs/22/32/33' => 'Метрична (PS)
1 HP = 1.014 метричних PS або CV',
  'specs/attrs/22/32/33/description' => 'Метрична (PS)
1 HP = 1.014 метричних PS або CV',
  'specs/attrs/22/32/34' => 'в діапазоні від',
  'specs/attrs/22/32/35' => 'в діапазоні до',
  'specs/attrs/22/32/171' => 'максимальна потужність (DIN)',
  'specs/attrs/22/32/171/description' => 'Потужність по DIN 70020. Європейський стандарт',
  'specs/attrs/22/32/172' => 'максимальна потужність (SAE certified)',
  'specs/attrs/22/32/172/description' => 'SAE Certified Power. Стандарт для США з 2005-06 року',
  'specs/attrs/22/32/173' => 'максимальна потужність (SAE net)',
  'specs/attrs/22/32/173/description' => 'Стандарт для США з 1971-72.
Трансмісія не враховується. Навісне обладнання - враховується',
  'specs/attrs/22/32/174' => 'максимальна потужність (SAE gross)',
  'specs/attrs/22/32/174/description' => 'Стандарт США 1972 року, приблизно 
відповідний технології виміру потужності до 1972.
Трансмісія - не враховується',
  'specs/attrs/22/32/177' => 'максимальна потужність (JIS D 1001)',
  'specs/attrs/22/32/178' => 'максимальна потужність (ДЕСТ)',
  'specs/attrs/22/32/178/description' => 'Стандарт СРСР і Росії',
  'specs/attrs/22/36' => 'крутний момент',
  'specs/attrs/22/36/37' => 'крутний момент',
  'specs/attrs/22/36/38' => 'в діапазоні від',
  'specs/attrs/22/36/39' => 'в діапазоні до',
  'specs/attrs/22/99' => 'турбонаддув',
  'specs/attrs/22/99/options/46' => 'нема',
  'specs/attrs/22/99/options/47' => 'є',
  'specs/attrs/22/99/options/48' => '×2',
  'specs/attrs/22/99/options/54' => '×3',
  'specs/attrs/22/99/options/49' => '×4',
  'specs/attrs/engine/turbo/options/x6' => '×6',
  'specs/attrs/22/156' => 'матеріал блоку',
  'specs/attrs/22/156/options/68' => 'чавун',
  'specs/attrs/22/156/options/69' => 'алюмінієвий сплав',
  'specs/attrs/22/156/options/70' => 'сталь',
  'specs/attrs/22/156/options/83' => 'магнієвий сплав',
  'specs/attrs/22/179' => 'охолодження',
  'specs/attrs/22/179/options/81' => 'повітряне',
  'specs/attrs/22/179/options/82' => 'рідинне',
  'specs/attrs/22/179/options/liquid-air' => 'повітряно-рідинне',
  'specs/attrs/22/206' => 'Газорозподільний механізм',
  'specs/attrs/22/206/options/88' => 'Кулачковий вал в блоці циліндрів',
  'specs/attrs/22/206/options/89' => 'Кулачковий вал в головці циліндрів',
  'specs/attrs/22/206/options/90' => 'Десмодромний ГРМ',
  'specs/attrs/22/206/options/91' => 'ГРМ без кулачкового валу',
  'specs/attrs/22/206/options/92' => 'Безклапанний',
  'specs/attrs/22/206/options/93' => 'Нижньоклапанний',
  'specs/attrs/22/206/options/94' => 'Зі змішаним розміщенням клапанів',
  'specs/attrs/22/206/options/95' => 'OHV',
  'specs/attrs/22/206/options/100' => 'OHV із системою зміни фаз газорозподілення',
  'specs/attrs/22/206/options/96' => 'SOHC',
  'specs/attrs/22/206/options/97' => 'SOHC із системою зміни фаз газорозподілення',
  'specs/attrs/22/206/options/98' => 'DOHC',
  'specs/attrs/22/206/options/99' => 'DOHC із системою зміни фаз газорозподілення',
  'specs/attrs/22/207' => 'тип',
  'specs/attrs/22/207/options/102' => 'Паровий',
  'specs/attrs/22/207/options/103' => 'Внутрішнього згорання',
  'specs/attrs/22/207/options/104' => 'Електричний',
  'specs/attrs/22/207/options/105' => 'Поршневий',
  'specs/attrs/22/207/options/106' => 'Роторно-поршневий',
  'specs/attrs/22/207/options/107' => 'Турбінний',
  'specs/attrs/40' => 'трансмісія',
  'specs/attrs/40/41' => 'привід',
  'specs/attrs/40/41/options/14' => 'передній',
  'specs/attrs/40/41/options/15' => 'задній',
  'specs/attrs/40/41/options/16' => 'повний',
  'specs/attrs/40/41/options/56' => 'передній на одне колесо',
  'specs/attrs/40/41/options/57' => 'задній на одне колесо',
  'specs/attrs/40/41/options/17' => 'повний з відкл. заднім',
  'specs/attrs/40/41/options/18' => 'повний з відкл. переднім',
  'specs/attrs/40/41/options/19' => 'постійний повний',
  'specs/attrs/40/42' => 'коробка передач',
  'specs/attrs/40/42/43' => 'тип',
  'specs/attrs/40/42/43/options/20' => 'МКПП',
  'specs/attrs/40/42/43/options/21' => 'АКПП',
  'specs/attrs/40/42/43/options/22' => 'CVT',
  'specs/attrs/40/42/43/options/23' => 'напівавтомат',
  'specs/attrs/40/42/43/options/50' => 'роботизована',
  'specs/attrs/40/42/43/options/87' => 'секвентальна',
  'specs/attrs/40/42/43/options/51' => 'DSG',
  'specs/attrs/40/42/43/options/52' => 'DCT',
  'specs/attrs/40/42/139' => 'назва',
  'specs/attrs/40/42/44' => 'кількість передач',
  'specs/attrs/40/83' => 'зчеплення',
  'specs/attrs/15' => 'підвіска і рульове управління',
  'specs/attrs/15/208' => 'передня підвіска',
  'specs/attrs/15/208/209' => 'тип пружного елементу',
  'specs/attrs/15/208/209/options/108' => 'пружинна',
  'specs/attrs/15/208/209/options/109' => '
ресорна',
  'specs/attrs/15/208/209/options/110' => 'пневматична',
  'specs/attrs/15/208/209/options/111' => 'гідропневматична',
  'specs/attrs/15/208/209/options/112' => 'торсіонна',
  'specs/attrs/15/208/209/options/113' => 'на резинових пружних елементах',
  'specs/attrs/15/208/209/options/178' => 'відсутня',
  'specs/attrs/15/208/209/options/114' => 'пружинна з штовхаючою штангою (Push-Rod)',
  'specs/attrs/15/208/209/options/115' => 'пружинна з тягнучою штангою (Pull-Rod)',
  'specs/attrs/15/208/209/options/116' => 'ресорна на поперечних ресорах',
  'specs/attrs/15/208/209/options/117' => 'ресорна на поздовжніх ресорах',
  'specs/attrs/15/208/209/options/124' => 'торсіонна з штовхаючою штангою (Push-Rod)',
  'specs/attrs/15/208/209/options/125' => 'торсіонна з тягнучою штангою (Pull-Rod)',
  'specs/attrs/15/208/209/options/118' => 'ресорна на поздовжніх еліптичних ресорах',
  'specs/attrs/15/208/209/options/119' => 'ресорна на поздовжніх ¾-еліптичних ресорах',
  'specs/attrs/15/208/209/options/120' => 'ресорна на поздовжніх ¼-еліптичних ресорах',
  'specs/attrs/15/208/209/options/121' => 'ресорна на поздовжніх напівеліптичних ресорах',
  'specs/attrs/15/208/209/options/122' => 'ресорна на поздовжніх кантилеверних ресорах',
  'specs/attrs/15/208/209/options/123' => 'ресорна на поздовжніх балансирних ресорах',
  'specs/attrs/15/208/210' => 'тип направляючого приладу',
  'specs/attrs/15/208/210/options/126' => 'залежна',
  'specs/attrs/15/208/210/options/127' => 'незалежна',
  'specs/attrs/15/208/210/options/128' => 'напівзалежна',
  'specs/attrs/15/208/210/options/129' => 'залежна безважільна',
  'specs/attrs/15/208/210/options/130' => 'залежна з поздовжніми важелями',
  'specs/attrs/15/208/210/options/131' => 'залежна з впираючою трубою або дишлом (Torque-tube або Torque-Rod)',
  'specs/attrs/15/208/210/options/132' => 'незалежна з коливаючимися напівосями',
  'specs/attrs/15/208/210/options/133' => 'незалежна на поздовжніх важелях',
  'specs/attrs/15/208/210/options/134' => 'незалежна підвіска Дюбонне',
  'specs/attrs/15/208/210/options/135' => 'незалежна на кривих важелях',
  'specs/attrs/15/208/210/options/136' => 'незалежна на подвійних поперечних важелях',
  'specs/attrs/15/208/210/options/137' => 'незалежна багатоважільна',
  'specs/attrs/15/208/210/options/138' => 'незалежна свічкова',
  'specs/attrs/15/208/210/options/139' => 'напівзалежна Де Діон',
  'specs/attrs/15/208/210/options/140' => 'напівзалежна торсіонно-важельна',
  'specs/attrs/15/208/210/options/141' => 'залежна з поздовжніми важелями з механізмом Уатта',
  'specs/attrs/15/208/210/options/142' => 'залежна з поздовжніми важелями з механізмом Скотта-Рассела',
  'specs/attrs/15/208/210/options/143' => 'залежна з поздовжніми важелями з тягою Панара',
  'specs/attrs/15/208/210/options/144' => 'залежна з упорною трубою або дишлом (Torque-tube або Torque-Rod) з механізмом Уатта',
  'specs/attrs/15/208/210/options/145' => 'залежна з упорною трубою або дишлом (Torque-tube або Torque-Rod) з механізмом Скотта-Рассела',
  'specs/attrs/15/208/210/options/146' => 'залежна з упорною трубою або дишлом (Torque-tube або Torque-Rod) з тягою Панара',
  'specs/attrs/15/208/210/options/147' => 'незалежна свічкова з коливаючою свічкою (McPherson)',
  'specs/attrs/15/208/210/options/149' => 'напівзалежна торсіонно-важельна з механізмом Уатта',
  'specs/attrs/15/208/210/options/150' => 'напівзалежна торсіонно-важельна з механізмом Скотта-Рассела',
  'specs/attrs/15/208/210/options/151' => 'напівзалежна торсіонно-важельна з тягою Панара',
  'specs/attrs/15/208/210/options/148' => 'незалежна свічкова з коливаючою свічкою (McPherson) на поздовжніх і поперечних важелях',
  'specs/attrs/15/208/211' => 'амортизатори',
  'specs/attrs/15/208/211/213' => 'наявність',
  'specs/attrs/15/208/211/214' => 'характер дії',
  'specs/attrs/15/208/211/214/options/152' => 'односторонній',
  'specs/attrs/15/208/211/214/options/153' => 'двосторонній',
  'specs/attrs/15/208/211/215' => 'принцип ції',
  'specs/attrs/15/208/211/215/options/154' => 'фрікціонний (механічний)',
  'specs/attrs/15/208/211/215/options/155' => 'гідравлічний',
  'specs/attrs/15/208/211/215/options/159' => 'газомасловий (олеопневматичний)',
  'specs/attrs/15/208/211/215/options/156' => 'гідравлічний важільний',
  'specs/attrs/15/208/211/215/options/157' => 'гідравлічний двотрубний',
  'specs/attrs/15/208/211/215/options/158' => 'гідравлічний однотрубний',
  'specs/attrs/15/208/211/216' => 'регульованість',
  'specs/attrs/15/208/212' => 'стабілізатор поперечної стійкості',
  'specs/attrs/15/8' => 'тип передньої підвіски (заст.)',
  'specs/attrs/15/9' => 'тип задньої підвіски (заст.)',
  'specs/attrs/15/10' => 'тип рульового управління',
  'specs/attrs/15/217' => 'задня підвіска',
  'specs/attrs/15/217/218' => 'тип пружного елементу',
  'specs/attrs/15/217/218/options/160' => 'пружинна',
  'specs/attrs/15/217/218/options/163' => 'ресорна',
  'specs/attrs/15/217/218/options/172' => 'пневматична',
  'specs/attrs/15/217/218/options/173' => 'гідропневматчна',
  'specs/attrs/15/217/218/options/174' => 'торсіонна',
  'specs/attrs/15/217/218/options/177' => 'на резинових пружних елементах',
  'specs/attrs/15/217/218/options/179' => 'відсутня',
  'specs/attrs/15/217/218/options/161' => 'пружинна з штовхаючою штангою (Push-Rod)',
  'specs/attrs/15/217/218/options/162' => 'пружинна з тягнучою штангою (Pull-Rod)',
  'specs/attrs/15/217/218/options/164' => 'ресорна на поперечних ресорах',
  'specs/attrs/15/217/218/options/165' => 'ресорна на поздовжніх ресорах',
  'specs/attrs/15/217/218/options/166' => 'ресорна на поздовжніх еліптичних ресорах',
  'specs/attrs/15/217/218/options/167' => 'ресорна на поздовжніх ¾-еліптичних ресорах',
  'specs/attrs/15/217/218/options/168' => 'ресорна на поздовжніх ¼-еліптичних ресорах',
  'specs/attrs/15/217/218/options/169' => 'ресорна на поздовжніх напівеліптичних ресорах',
  'specs/attrs/15/217/218/options/170' => 'ресорна на поздовжніх кантилеверних ресорах',
  'specs/attrs/15/217/218/options/175' => 'торсіонна з штовхаючою штангою (Push-Rod)',
  'specs/attrs/15/217/218/options/171' => 'ресорна на поздовжніх балансуючих ресорах',
  'specs/attrs/15/217/218/options/176' => 'торсіонна з тягнучою штангою (Pull-Rod)',
  'specs/attrs/15/217/219' => 'тип направляючого приладу',
  'specs/attrs/15/217/219/options/180' => 'залежна',
  'specs/attrs/15/217/219/options/190' => 'незалежна',
  'specs/attrs/15/217/219/options/200' => 'напівзалежна',
  'specs/attrs/15/217/219/options/181' => 'залежна безважільна',
  'specs/attrs/15/217/219/options/182' => 'залежна з поздовжніми важелями',
  'specs/attrs/15/217/219/options/186' => 'залежна з упорною трубою або дишлом (Torque-tube або Torque-Rod)',
  'specs/attrs/15/217/219/options/183' => 'залежна з поздовжніми важелями з механізмом Уатта',
  'specs/attrs/15/217/219/options/184' => 'залежна з поздовжніми важелями з механізмом Скотта-Рассела',
  'specs/attrs/15/217/219/options/185' => 'залежна з поздовжніми важелями з тягою Панара',
  'specs/attrs/15/217/219/options/187' => 'залежна з упорною трубою або дишлом Torque-tube або Torque-Rod) з механізмом Уатта',
  'specs/attrs/15/217/219/options/188' => 'залежна з упорною трубою або дишлом Torque-tube або Torque-Rod) з механізмом Скотта-Рассела',
  'specs/attrs/15/217/219/options/189' => 'залежна з упорною трубою або дишлом Torque-tube або Torque-Rod) з тягою Панара',
  'specs/attrs/15/217/219/options/191' => 'незалежна з хилитаючимися напівосями',
  'specs/attrs/15/217/219/options/192' => 'незалежна на поздовжніх важелях',
  'specs/attrs/15/217/219/options/193' => 'незалежна підвіска Дюбонне',
  'specs/attrs/15/217/219/options/194' => 'незалежна на кривих важелях',
  'specs/attrs/15/217/219/options/195' => 'незалежна на подвійних поперечних важелях',
  'specs/attrs/15/217/219/options/196' => 'незалежна багатоважільна',
  'specs/attrs/15/217/219/options/197' => 'незалежна свічкова',
  'specs/attrs/15/217/219/options/198' => 'незалежна свічкова з хилитаючоюся  свічкою (McPherson)',
  'specs/attrs/15/217/219/options/199' => 'незалежна свічкова з хилитаючоюся  свічкою (McPherson) на поздовжніх і поперечних важелях',
  'specs/attrs/15/217/219/options/201' => 'напівзалежна Де Діон',
  'specs/attrs/15/217/219/options/202' => 'напівзалежна торсіонно-важільна',
  'specs/attrs/15/217/219/options/203' => 'напівзалежна торсіонно-важільна з механізмом Уатта',
  'specs/attrs/15/217/219/options/204' => 'напівзалежна торсіонно-важільна з механізмом Скотта-Рассела',
  'specs/attrs/15/217/219/options/205' => 'напівзалежна торсіонно-важільна з тягою панара',
  'specs/attrs/15/217/220' => 'амортизатори',
  'specs/attrs/15/217/220/222' => 'наявність',
  'specs/attrs/15/217/220/223' => 'характер дії',
  'specs/attrs/15/217/220/223/options/206' => 'односторонній',
  'specs/attrs/15/217/220/223/options/207' => 'двосторонній',
  'specs/attrs/15/217/220/224' => 'принцип дії',
  'specs/attrs/15/217/220/224/options/208' => 'фрикційний (механічний)',
  'specs/attrs/15/217/220/224/options/209' => 'гідравлічний',
  'specs/attrs/15/217/220/224/options/213' => 'газомасляний (олеопневматичний)',
  'specs/attrs/15/217/220/224/options/210' => 'гідравлічний важільний',
  'specs/attrs/15/217/220/224/options/211' => 'гідравлічний однотрубний',
  'specs/attrs/15/217/220/224/options/212' => 'гідравлічний двотрубний',
  'specs/attrs/15/217/220/225' => 'регульованість',
  'specs/attrs/15/217/221' => 'стабілізатор поперечної стійкості',
  'specs/attrs/74' => 'гальмівна система',
  'specs/attrs/74/77' => 'ABS',
  'specs/attrs/74/142' => 'передні гальма',
  'specs/attrs/74/142/75' => 'опис',
  'specs/attrs/74/142/144' => 'тип',
  'specs/attrs/74/142/144/options/58' => 'барабанні',
  'specs/attrs/74/142/144/options/59' => 'дискові',
  'specs/attrs/74/142/146' => 'діаметр',
  'specs/attrs/74/142/148' => 'товщина',
  'specs/attrs/74/142/150' => 'матеріал',
  'specs/attrs/74/142/150/options/62' => 'метал',
  'specs/attrs/74/142/150/options/63' => 'карбон',
  'specs/attrs/74/142/150/options/64' => 'кераміка',
  'specs/attrs/74/142/152' => 'вентильовані',
  'specs/attrs/74/142/153' => 'перфоровані',
  'specs/attrs/74/143' => 'задні гальма',
  'specs/attrs/74/143/76' => 'опис',
  'specs/attrs/74/143/145' => 'тип',
  'specs/attrs/74/143/145/options/60' => 'барабанні',
  'specs/attrs/74/143/145/options/61' => 'дискові',
  'specs/attrs/74/143/147' => 'діаметр',
  'specs/attrs/74/143/149' => 'товщина',
  'specs/attrs/74/143/151' => 'матеріал',
  'specs/attrs/74/143/151/options/65' => 'метал',
  'specs/attrs/74/143/151/options/66' => 'карбон',
  'specs/attrs/74/143/151/options/67' => 'кераміка',
  'specs/attrs/74/143/154' => 'вентильовані',
  'specs/attrs/74/143/155' => 'перфоровані',
  'specs/attrs/181' => 'електрика',
  'specs/attrs/181/182' => 'напруга бортової мережі',
  'specs/attrs/46' => 'динамічні характеристики',
  'specs/attrs/46/47' => 'максимальна швидкість',
  'specs/attrs/46/180' => 'розгін до 60 км/год',
  'specs/attrs/46/48' => 'розгін до 100 км/год',
  'specs/attrs/46/49' => 'розгін до 200 км/год',
  'specs/attrs/46/175' => 'розгін до 60 миль/год',
  'specs/attrs/46/50' => 'розгін до 300 км/год',
  'specs/attrs/46/51' => 'час пробігу 400 м',
  'specs/attrs/46/52' => 'час пробігу 1000 м',
  'specs/attrs/46/53' => 'обмежувач швидкості',
  'specs/attrs/46/160' => 'час гальмування  зі 100 км/год',
  'specs/attrs/46/161' => 'гальмівний шлях зі 100 км/год',
  'specs/attrs/54' => 'експлуатаційні характеристики',
  'specs/attrs/54/55' => 'кут виїзду',
  'specs/attrs/54/56' => 'кут з\'їзду',
  'specs/attrs/54/57' => 'об\'єм баку пального',
  'specs/attrs/54/57/58' => 'основний',
  'specs/attrs/54/57/59' => 'додатковий',
  'specs/attrs/54/60' => 'об\'єм багажнику',
  'specs/attrs/54/60/61' => 'мін',
  'specs/attrs/54/60/62' => 'макс',
  'specs/attrs/54/78' => 'витрати пального',
  'specs/attrs/54/78/183' => 'Невстановлений метод',
  'specs/attrs/54/78/183/79' => 'в місті',
  'specs/attrs/54/78/183/80' => 'на трасі',
  'specs/attrs/54/78/183/81' => 'змішаний режим',
  'specs/attrs/54/78/184' => 'ECE',
  'specs/attrs/54/78/184/185' => '90 км/год',
  'specs/attrs/54/78/184/186' => '120 км/год',
  'specs/attrs/54/78/184/187' => 'city',
  'specs/attrs/54/78/184/188' => 'combined',
  'specs/attrs/54/78/189' => 'EPA (до 2008)',
  'specs/attrs/54/78/189/190' => 'city',
  'specs/attrs/54/78/189/191' => 'highway',
  'specs/attrs/54/78/192' => 'EPA (з 2008)',
  'specs/attrs/54/78/192/193' => 'city',
  'specs/attrs/54/78/192/194' => 'highway',
  'specs/attrs/54/78/199' => 'EU 93/116/EC',
  'specs/attrs/54/78/199/200' => 'urban',
  'specs/attrs/54/78/199/201' => 'extra urban',
  'specs/attrs/54/78/199/202' => 'combined',
  'specs/attrs/54/78/199/202/description' => '36.8% urban + 63.2% extra urban',
  'specs/attrs/54/138' => 'Система "старт-стоп"',
  'specs/attrs/54/158' => 'вантажопідйомність',
  'specs/attrs/54/205' => 'вага буксованого причепу',
  'specs/attrs/54/226' => 'об\'єм кузову',
  'specs/attrs/54/195' => 'розворот',
  'specs/attrs/54/195/11' => 'діаметр',
  'specs/attrs/54/195/11/description' => 'осьовий. 2×радіус',
  'specs/attrs/54/195/196' => 'від стіни до стіни',
  'specs/attrs/54/195/196/description' => '',
  'specs/attrs/54/195/197' => 'від бордюру до бордюру',
  'specs/attrs/54/195/197/description' => '',
  'specs/attrs/54/198' => 'оборотів руля від упору до упору',
  'specs/attrs/84' => 'колеса і диски',
  'specs/attrs/84/85' => 'передні',
  'specs/attrs/84/85/87' => 'ширина покришок',
  'specs/attrs/84/85/90' => 'висота покришок',
  'specs/attrs/84/85/88' => 'діаметр',
  'specs/attrs/84/85/89' => 'ширина диску',
  'specs/attrs/84/85/162' => 'виліт диску',
  'specs/attrs/84/86' => 'задні',
  'specs/attrs/84/86/91' => 'ширина покришок',
  'specs/attrs/84/86/94' => 'висота покришок',
  'specs/attrs/84/86/92' => 'діаметр',
  'specs/attrs/84/86/93' => 'ширина диску',
  'specs/attrs/84/86/163' => 'виліт диску',
  'specs/attrs/84/164' => 'модель дисків',
  'specs/attrs/84/165' => 'матеріал дисків',
  'specs/attrs/84/165/options/78' => 'сталь',
  'specs/attrs/84/165/options/79' => 'алюмінієвий сплав (лиття)',
  'specs/attrs/84/165/options/80' => 'алюмінієвий сплав (ковка)',
  'specs/attrs/82' => 'шкідливі викиди',
  'specs/attrs/157' => 'норма викидів',
  'specs/attrs/157/options/71' => 'Euro 1',
  'specs/attrs/157/options/72' => 'Euro 2',
  'specs/attrs/157/options/73' => 'Euro 3',
  'specs/attrs/157/options/74' => 'Euro 4',
  'specs/attrs/157/options/75' => 'Euro 5',
  'specs/attrs/157/options/76' => 'Euro 5+',
  'specs/attrs/157/options/77' => 'Euro 6',
  'specs/attrs/170' => 'місце виробництва',
  'specs/unit/1/abbr' => 'мм',
  'specs/unit/1/name' => 'міліметр',
  'specs/unit/2/abbr' => 'кг',
  'specs/unit/2/name' => 'кілограм',
  'specs/unit/3/abbr' => 'м',
  'specs/unit/3/name' => 'метр',
  'specs/unit/4/abbr' => 'см³',
  'specs/unit/4/name' => 'кубічний сантиметр',
  'specs/unit/5/abbr' => 'к.с.',
  'specs/unit/5/name' => 'коняча сила',
  'specs/unit/6/abbr' => 'об/хв',
  'specs/unit/6/name' => 'оборотів в хвилину',
  'specs/unit/7/abbr' => 'Нм',
  'specs/unit/7/name' => 'Ньютон-метр',
  'specs/unit/8/abbr' => 'км/год',
  'specs/unit/8/name' => 'кілометрів на годину',
  'specs/unit/9/abbr' => 'сек',
  'specs/unit/9/name' => 'секунда',
  'specs/unit/10/abbr' => '%',
  'specs/unit/10/name' => 'відсоток',
  'specs/unit/11/abbr' => '°',
  'specs/unit/11/name' => 'градус',
  'specs/unit/12/abbr' => 'л',
  'specs/unit/12/name' => 'літр',
  'specs/unit/13/abbr' => 'л/100 км',
  'specs/unit/13/name' => 'літрів на 100 кілометрів',
  'specs/unit/14/abbr' => 'CO2 г/км',
  'specs/unit/14/name' => 'грам CO2 на кілометр',
  'specs/unit/15/abbr' => '″',
  'specs/unit/15/name' => 'дюйм',
  'specs/unit/16/abbr' => 'р.',
  'specs/unit/16/name' => 'рік',
  'specs/unit/17/abbr' => 'PS',
  'specs/unit/17/name' => 'Pferdestärke',
  'specs/unit/18/abbr' => 'кВт',
  'specs/unit/18/name' => 'кіловатт',
  'specs/unit/19/abbr' => 'V',
  'specs/unit/19/name' => 'вольт',
  'specs/unit/20/abbr' => 'м³',
  'specs/unit/20/name' => 'м³',
  'telegram/info' => '[Telegram](https://telegram.org/) - це в першу чергу система обміну миттєвими повідомленнями, подібна whatsapp або viber.

Крім звичного обміну повідомленнями і файлами, тут є багато інших можливостей, на опис яких піде багато часу.

В роботі сайту ми використовуємо лише дві з них: публічні групи і боти.

# Публічна група

Є відкриті російськомовні і англомовні групи для всіх користувачів сайту.

Ввійти в них дуже просто - достатньо перейти за посиланням і слідувати інструкціям:

* [російськомовний](https://t.me/joinchat/AAAAAA0NvB5g7SEsWv61Rw)
* [англомовний](https://t.me/joinchat/AAAAAAvxJESUMQcUM-I5YA)

# Бот

Для вашої і нашої зручності ми створили "бота" в додатку [telegram](https://telegram.org/), який дозволить покращити ваш досвід взаємодії з сайтом.

Звати його просто: [@autowp_bot](tg:msg).

Якщо коротко: бот, це користувач телеграм, який може надсилати вам повідомлення, підготовлені нашим сайтом.

Детальніше про ботів telegram можна почитати тут <https://core.telegram.org/bots/faq>

На даний момент підтримується 2 близькі функці:

* Повідомлення про нові особисті повідомлення. [Детальніше ...](#messages)
* Повідомлення про нові прийняті на сайт зображення. [Детальніше ...](#new)
* Повідомлення про нові завантажені (але ще не прийняті) зображення (тільки для зареєстрованих користувачів). [Детальніше ...](#inbox)

## Команди бота

### Старт: `/start`

Відправивши цю команду у відповідь ви отримаєте список підтримуваних команд.

### Особисті повідомлення: `/messages`

Відправте `/messages on`, щоб підписатися на отримання повідомлень.

Відправте `/messages off`, щоб відписатися.

### Нові зображення: `/new`

Відправте `/new BMW`, щоб підписатися на отримання нових фотографій BMW.

Відправте `/new BMW` повторно, щоб відписатися.

### Інбокс: `/inbox`

Потрібна попередня авторизація командою `/me`

Відправте `/inbox BMW`, щоб підписатися на отримання повідомлень про нові завантажені, але ще не прийняті фотографії BMW.

Відправте `/inbox BMW` повторно, щоб відписатися.

### Я (авторизація): `/me`

Ця команда дозволяє вам асоціювати себе (користувача telegram) з обліковим записом на нашому сайті.

Відправте `/me`, щоб отримати інструкції по виконанню авторизації.

Відправте `/me 123456789`, щоб отримати код підтвердження.

Тут 123456789 - номер вашого облікового запису на нашому сайті, який ви можете знайти на сторінці свого профілю. Наприклад, номер [Juliano Scotini](/users/juliano-scotini): 17322

Код підтвердження буде відправлений вам через [систему особистих повідомлень](/account/messages?folder=system)

Відправте `/me 123456789 ХХХХХХХХХХХХХХХХ`, щоб завершити процес асоціації вашого облікового запису telegram з користувачем нашого сайту.

Тут `ХХХХХХХХХХХХХХХХ` - код, який буде відправлений вам через [систему особистих повідомлень](/account/messages?folder=system)',
  'personal-message-dialog/title' => 'Відправити особисте повідомлення',
  'personal-message-dialog/send' => 'відправити',
  'personal-message-dialog/sending' => 'відправляється ...',
  'personal-message-dialog/sent' => 'відправлено',
  'personal-message-dialog/cancel' => 'відмінити',
  'personal-message-dialog/placeholder' => 'Текст повідомлення',
  'crop-dialog/title' => 'Кадрування',
  'dialog/close' => 'Закрити',
  'crop-dialog/select-all' => 'Виділити всі',
  'crop-dialog/save' => 'Зберегти',
  'crop-dialog/resolution-%s-aspect-%s' => '%s (пропорції %s)',
  'crop-dialog/resolution-x-aspect-y' => '{resolution} (пропорції {aspect})',
  'who-online/title' => 'Онлайн',
  'who-online/refresh' => 'Оновити',
  'picture-moder-vote/custom/title' => 'Своя причина',
  'picture-moder-vote/custom/sending' => 'Відправляється ...',
  'picture-moder-vote/custom/send' => 'Відправлено',
  'picture-moder-vote/custom/cancel' => 'Відмінити',
  'picture-moder-vote/custom/save' => 'Зберегти як шаблон',
  'moder/statistics/photos-with-copyrights' => 'Фотографій з копірайтами',
  'moder/statistics/vehicles-with-4-or-more-photos' => 'Автомобілів з 4 і більше фото',
  'moder/statistics/specifications-values' => 'Заповнення значень ТТХ',
  'moder/statistics/brand-logos' => 'Логотипів брендів',
  'moder/statistics/from-years' => 'Роки початку випуску автомобілів',
  'moder/statistics/from-and-to-years' => 'Роки початку і закінчення випуску автомобілів',
  'moder/statistics/from-and-to-years-and-months' => 'Роки і місяці початку і закінчення випуску автомобілів',
  'forums/go-to-theme-name' => 'Зайти у форум "{theme_name}"',
  'forums/go-to-theme' => 'Зайти у форум …',
  'forums/subforums' => 'Під-форуми',
  'forums/last-message-in-topic-{topic_name}' => 'Останнє повідомлення в темі «{topic_name}»',
  'forums/go-to-last-message' => 'Перейти до останнього повідомлення',
  'forums/create-new-topic' => 'Створити нову тему',
  'forums/theme/topics' => 'Теми',
  'forums/theme/author' => 'Автор',
  'forums/theme/last-message' => 'Останнє повідомлення',
  'forums/topic/open' => 'відкрити',
  'forums/topic/close' => 'закрити',
  'forums/topic/delete' => 'видалити',
  'forums/topic/move' => 'перемістити',
  'forums/topic/name' => 'Тема топіку',
  'forums/topic/text' => 'Повідомлення',
  'forums/topic/subscribe-to-new-messages' => 'Підписатись на нові повідомлення',
  'forums/theme/subscription' => 'Підписка',
  'forums/unsubscribe' => 'Відписатися',
  'forums/theme/total:' => 'Всього',
  'forums/theme/all-other' => 'Про все',
  'forums/theme/photos' => 'Фото-теми',
  'forums/theme/classifier' => 'Класифікатор',
  'forums/theme/this-cite' => 'Робота сайту',
  'forums/theme/german-cars' => 'Німецькі автомобілі',
  'forums/theme/italian-cars' => 'Італійські автомобілі',
  'forums/theme/french-cars' => 'Французькі автомобілі',
  'forums/theme/japan-cars' => 'Японські автомобілі',
  'forums/theme/korean-cars' => 'Корейські автомобілі',
  'forums/theme/chinese-cars' => 'Китайські автомобілі',
  'forums/theme/russian-cars' => 'Російські автомобілі',
  'forums/theme/american-cars' => 'Американські автомобілі',
  'forums/theme/british-cars' => 'Британські автомобілі',
  'forums/theme/moderators' => 'Модераторський форум',
  'forums/theme/cars' => 'Автомобілі',
  'forums/topic-closed-text' => 'Обговорення закрито адміністрацією форуму',
  'forums/register-to-post' => 'Щоб мати можливість створювати теми і відповідати в них, Вам необхідно [зареєструватися](/signup).',
  'forums/register-to-create-topic' => 'Додавати топіки можуть тільки зареєстровані користувачі.

Якщо ви вже зареєстровані на сайті, то вам необхідно здійснити вхід на сайт.

Якщо ви ще не зареєстровані, зробіть це зараз [на сторінці реєстрації](/signup).',
);