...
class Catalogue extends AbstractRoute
{
    ...
    public function match($path)
    {
        ...
        if ($dpBrand) {
            ...
            if (!$path) {
                ...
                return $this->_assembleMatch(array(
                    ...
                    'brand_catname'   => $brand->folder,
                    'dpbrand_catname' => $dpBrand->folder
                ));
            }
        ...
        $dps = new Design_Projects();
        $designProject = $dps->fetchRow(array(
            'brand_id = ?' => $brand->id,
            'catname = ?'  => $path[0]
        ));
        if ($designProject) {
            ...
            if (!$path) {
                // :brand/:design_project
                return $this->_assembleMatch(array(
                    'action'                 => 'design-project-cars',
                    'brand_catname'          => $brand->folder,
            ...
            if ($path[0] == 'gallery') {
                ...
                if (!$path) {
                    // :brand/:design_project/gallery
                    return $this->_assembleMatch(array(
                        'action'                 => 'design-project-gallery',
                        'brand_catname'          => $brand->folder,
                        'design_project_catname' => $designProject->catname,
                    ));
                }
                ...
                if (in_array($path[0], ['inbox', 'removing'])) {
                    ...
                    if (!$path) {
                        // :brand/:design_project/gallery/:gallery
                        return $this->_assembleMatch(array(
                            'action'                 => 'design-project-gallery',
                            'brand_catname'          => $brand->folder,
                            'design_project_catname' => $designProject->catname,
            ...
            if (!$path) {
                // :brand/:design_project/:picture
                return $this->_assembleMatch(array(
                    'action'                 => 'design-project-picture',
                    'brand_catname'          => $brand->folder,
                    'design_project_catname' => $designProject->catname,
                    'picture_id'             => $pictureId
                ));
    ...
    public function assemble($data = array(), $reset = false, $encode = false)
    {
        ...
        switch ($data['action'])
        {
            ...
            case 'design-project-brand':
                $url[] = $data['dpbrand_catname'];
                if (isset($data['page']) && $data['page'] > 1)
                    $url[] = 'page' . $data['page'];
                break;

            case 'design-project':
                $url[] = $data['design_project_catname'];
                break;

            case 'design-project-cars':
                $url[] = $data['design_project_catname'];
                if (isset($data['page']) && $data['page'] > 1)
                    $url[] = 'page' . $data['page'];
                break;

            case 'design-project-pictures':
                $url[] = $data['design_project_catname'];
                $url[] = 'pictures';
                if (isset($data['page']) && $data['page'] > 1)
                    $url[] = 'page' . $data['page'];
            ...
            case 'design-project-picture':
                $url[] = $data['design_project_catname'];
                $url[] = $data['picture_id'];
                break;

            case 'design-project-gallery':
                $url[] = $data['design_project_catname'];
                $url[] = 'gallery';
                if (isset($data['gallery']) && $data['gallery']) {
                    $url[] = $data['gallery'];
